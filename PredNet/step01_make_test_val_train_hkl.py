...
categories = ['train','val','test'] #['city', 'residential', 'road']
# 
...
test_recordings = [('test', 'frames')] #[('city', '2011_09_26_drive_0104_sync'), ('residential', '2011_09_26_drive_0079_sync'), ('road', '2011_09_26_drive_0070_sync')]

def process_data():
    splits = {s: [] for s in ['train', 'test', 'val']}
    splits['val'] = val_recordings
    splits['test'] = test_recordings
    not_train = splits['test'] + splits['val'] 
    # for c in categories:  # Randomly assign recordings to training and testing. Cross-validation done across entire recordings.
    #     c_dir = os.path.join(ball_DATA_DIR, c + '/') # 'raw', c + '/')
    #     folders= list(os.walk(c_dir, topdown=False))[-1][-2]
    #     splits['train'] += [(c, f) for f in folders if (c, f)  not in not_train]

    for split in splits:
        # if split == 'test':  # only when one of test,val,train hkl is needed
        im_list = []
        source_list = []  # corresponds to recording that image came from
        for category, folder in splits[split]:
            im_dir = os.path.join(ball_DATA_DIR, category,'frames/') # ,  'raw/', category, folder, folder[:10], folder, 'image_03/data/')
        ...
        hkl.dump(X, os.path.join(AlexNet_HICKLE_DUMP, 'X_' + split +  '.hkl'))
        hkl.dump(source_list, os.path.join(AlexNet_HICKLE_DUMP, 'sources_'+ split + '.hkl'))
