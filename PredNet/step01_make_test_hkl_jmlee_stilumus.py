...
stimulus_type = ['circle', 'garbor_frequency', 'garbor_orientation',  'garbor_phase',  'garbor_sc',  'movie']
session_type = ['predictable_20fps', 'unpredictable_20fps']
Trials = ['trial_{}'.format(i) for i in range(1, 15)]


categories = [
    '{}/{}/{}'.format(stimulus, session, trial)
    for stimulus in stimulus_type[5:6]
    ...
    for cat in categories:
        ...
        for split in splits:
            ...
            for category, folder in splits[split]:
                im_dir = os.path.join(Jlee_DATA_DIR, category,'frames/') # ,  'raw/', category, folder, folder[:10], folder, 'image_03/data/')
                print('im_dir: ' + im_dir)
            ...
            # jlee
            stimulus, session, trial = category.split('/')
            save_dir = os.path.join(Jlee_HICKLE_DUMP, stimulus, session)
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            
            hkl.dump(X, os.path.join(Jlee_HICKLE_DUMP, stimulus, session, 'X.hkl')) 
            hkl.dump(source_list, os.path.join(Jlee_HICKLE_DUMP, stimulus, session, 'sources.hkl'))
