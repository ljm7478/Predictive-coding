    ...
    for cat in categories:
        ...
        for split in splits:
            ...
            for category, folder in splits[split]:
                im_dir = os.path.join(BP_DATA_DIR, category,'frames/') # ,  'raw/', category, folder, folder[:10], folder, 'image_03/data/')
                print('im_dir: ' + im_dir)
            ...
            hkl.dump(X, os.path.join(GUMP_HICKLE_DUMP,'X_' + name +  '.hkl'))
            hkl.dump(source_list, os.path.join(GUMP_HICKLE_DUMP, 'sources_' + name +  '.hkl'))
