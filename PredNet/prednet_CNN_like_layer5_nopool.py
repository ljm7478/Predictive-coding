...
class PredNet(Recurrent):
    '''PredNet architecture - Lotter 2016.
        ...
        Modified for "CNN-like" architecture with "Nopool" logic:
        Pooling is limited to self.max_pool_layers (default 3) to maintain 
        spatial resolution in deeper layers.
    '''
    ...
    def __init__(self, stack_sizes, R_stack_sizes,
        ...
        if self.output_mode in layer_output_modes:
            self.output_layer_type = self.output_mode[:-1]                # jmlee; ex) R 
            self.output_layer_num = int(self.output_mode[-1])             # jmlee; ex) 0
        else:
        ...
        # --- REVISION: Define max pooling steps ---
        self.max_pool_layers = 3 # Pool at l=0, l=1, l=2 only
        # ------------------------------------------
    ...
    def compute_output_shape(self, input_shape):
        if self.output_mode == 'prediction':
            ...
            # --- REVISION: Calculate shape with nopool logic ---
        ...
        if self.return_sequences:
            return (input_shape[0], input_shape[1]) + out_shape  #jmlee input_shape:(None, 10, 128, 160, 3) out_shape: (4,)
        else:
    ...
    def get_initial_state(self, x):
        ...
        base_initial_state = K.zeros_like(x)  # (samples, timesteps) + image_shape #jmlee:Creates a tensor with all elements set to zero.

        non_channel_axis = -1 if self.data_format == 'channels_first' else -2
        for _ in range(2):   #jmlee, _: 0,1
        ...
        nlayers_to_pass = {u: self.nb_layers for u in states_to_pass}  #jmlee, {'c':4, 'e':4, 'r':4}
        ...
        for u in states_to_pass:
            for l in range(nlayers_to_pass[u]):    #jmlee, for l in [0,1,2,3]
                
                # --- REVISION: Nopool logic for initial states ---
                n_pool = min(l, self.max_pool_layers)
                ds_factor = 2 ** n_pool
                # -------------------------------------------------

                nb_row = init_nb_row // ds_factor  #jmlee for l=0 nb_row: 128, nb_col: 160
                nb_col = init_nb_col // ds_factor
                if u in ['r', 'c']:
                    stack_size = self.R_stack_sizes[l]
                elif u == 'a':
                    stack_size = self.stack_sizes[l] #2 * self.stack_sizes[l]
                elif u == 'ahat':
                    stack_size = self.stack_sizes[l]
                output_size = stack_size * nb_row * nb_col  # flattened size

                reducer = K.zeros((input_shape[self.channel_axis], output_size)) # (nb_channels, output_size) #jmlee, ex: (3,61440); 61440: 3*128*160
                initial_state = K.dot(base_initial_state, reducer) # (samples, output_size)    #jmlee, ex: (?,61440)
                if self.data_format == 'channels_first':
                    output_shp = (-1, stack_size, nb_row, nb_col)
                else:
                    output_shp = (-1, nb_row, nb_col, stack_size)       #jmlee, (-1,128,160,3)
                initial_state = K.reshape(initial_state, output_shp)   #jmlee, (?,128,160,3)
    ...
    def build(self, input_shape):
        ...
        for c in sorted(self.conv_layers.keys()):  #jmlee: 0:'a', 1:'c', 2:'f', 3:'i', 4:'o', 5:'ahat'
            for l in range(len(self.conv_layers[c])):
                
                # --- REVISION: ds_factor based on nopool logic ---
                n_pool = min(l, self.max_pool_layers)
                ds_factor = 2 ** n_pool
                # -------------------------------------------------
                ...
                print('{}_in_shape per {} is {}'.format(str(c), str(l), str(in_shape)))
    ...
    def step(self, a, states):
        r_tm1 = states[:self.nb_layers]                             #jmlee, [:4] -> [0,1,2,3]
        c_tm1 = states[self.nb_layers:2*self.nb_layers]             #jmlee, [4:8]-> [0,1,2,3]
        a_tm1 = states[2*self.nb_layers:3*self.nb_layers]           #jmlee, [8:12]-> [0,1,2,3]
        ...
        # Update R units starting from the top  #jmlee; layer3 -> 0 
        for l in reversed(range(self.nb_layers)):
            ...
            if l > 0:
                # --- REVISION: Conditional Upsampling (Top-Down) ---
                # Check if resolution changes between current layer l and lower layer l-1
                n_pool = min(l, self.max_pool_layers)
                n_pool_down = min(l - 1, self.max_pool_layers)

                if n_pool == n_pool_down:
                    r_up = _r  # Resolution is same, just pass it down
                else:
                    r_up = self.upsample.call(_r) # Resolution increases, upsample
                # ---------------------------------------------------
        ...
        for l in range(self.nb_layers):
            ...
            a_pass.append(a)  #a is appended to a_pass to use a[l-1] for next input target
            ...
            #jmlee: making the target for the next layer 
            if l < self.nb_layers - 1:
                if l == 0 :
                    a = self.conv_layers['a'][l].call(a)
                else:
                    a = self.conv_layers['a'][l].call(a_pass[l])
                
                # --- REVISION: Conditional Pooling (Bottom-Up) ---
                # Only pool if we haven't reached the max pooling depth yet
                if l < self.max_pool_layers:
                    a = self.pool.call(a)  # next_layer target shape is made
                # -------------------------------------------------
