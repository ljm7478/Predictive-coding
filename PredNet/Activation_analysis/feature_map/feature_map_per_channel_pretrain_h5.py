...
import math
import re
import h5py  # <<< Added to read HDF5 files

# --- Configuration ---
BASE_RESULTS_DIR = '/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/01_gump/result/prednet_original/h5/pt_kitti_ft_Lall_nt150/layer4'
PLOTS_SAVE_DIR = '/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/activation_verification_jm/featureMap/original_layer4/pre_trained_kitti_rev'
...
def compare_ahat_vs_e(layer_data_dict, run_name, timestep, save_dir):
    ...
    (This function already assumes channels-first data: [B, T, C, H, W])
    """
...
def visualize_all_channels(data, layer_name, timestep, save_dir, resize_to=(64, 64)):
    ...
    # CRITICAL: Assumes data is already in channels_first (B, T, C, H, W) format.
    # (This line is removed because transpose is done during data loading)
...
if __name__ == "__main__":
    ...
    for layer_type in layers_types:
        for layer_name in layers_to_load:
            # npy_path = os.path.join(run_dir, "{}.npy".format(layer_name)) # OLD
            h5_path = os.path.join(run_dir, "{}_{}.h5".format(RUN_TO_PLOT,layer_type)) # <<< MODIFIED: Changed to .h5

            # if os.path.exists(npy_path): # OLD
            if os.path.exists(h5_path): # <<< MODIFIED
                print("Loading {}...".format(h5_path)) # <<< MODIFIED
                
                # --- MODIFIED: HDF5 loading logic ---
                try:
                    with h5py.File(h5_path, 'r') as f:
                        # Assume the dataset name inside the HDF5 file is the same as the file name (e.g., 'Ahat0.h5' -> 'Ahat0')
                        # PredNet typically saves in (B, T, H, W, C) - channels-last format.
                        data_ch_last = f[layer_name][:]
                    
                    # Convert to channels-first (B, T, C, H, W) for consistency in visualization functions
                    data_ch_first = np.transpose(data_ch_last, (0, 1, 4, 2, 3))
                    layer_data_dict[layer_name] = data_ch_first
                    
                    print("  Loaded data shape (channels-last): {}, Transposed to (channels-first): {}".format(
                        data_ch_last.shape, data_ch_first.shape
                    ))

                except KeyError:
                    print("  Error: Dataset '{}' not found in {}. Skipping.".format(layer_name, h5_path))
                except Exception as e:
                    print("  Error loading {}: {}. Skipping.".format(h5_path, e))
                # --- END MODIFIED ---

            else:
                # print("Warning: Could not find {}. Skipping.".format(npy_path)) # OLD
                print("Warning: Could not find {}. Skipping.".format(h5_path)) # <<< MODIFIED
