...
pretrained_json_file = './model_data_keras2/prednet_kitti_model.json'   #channels_first
pretrained_weights_file = './model_data_keras2/tensorflow_weights/prednet_kitti_weights-Lall.hdf5'   #channels_first

# Where to save the fine-tuned model
save_model = True
output_dir = os.path.join(Titanic_WEIGHTS_DIR, 'prednet', 'pt_kitti_ft_Lall_nt150')
...
train_generator = SequenceGenerator(train_file, train_sources, nt, batch_size=batch_size, shuffle=True, data_format='channels_first')
val_generator = SequenceGenerator(val_file, val_sources, nt, batch_size=batch_size, N_seq=N_seq_val, data_format='channels_first')
