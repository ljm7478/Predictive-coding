# -*- coding: utf-8 -*-
'''
Train PredNet on KITTI sequences (5 Layers, L0 Loss).
'''

import os
import numpy as np
np.random.seed(123)

from keras import backend as K
from keras.models import Model
from keras.layers import Input, Dense, Flatten
from keras.layers import TimeDistributed
from keras.callbacks import LearningRateScheduler, ModelCheckpoint
from keras.optimizers import Adam

from prednet_layer5_nopool import PredNet
from data_utils import SequenceGenerator

# --- Configuration ---
DATA_DIR = './original_code/kitti_data'
WEIGHTS_DIR = './data/00_KITTI_pre-train/weights' # Unique directory for 5 layers

save_model = True
weights_file = os.path.join(WEIGHTS_DIR, 'prednet_kitti_5layers_nopool_weights_Lall.hdf5') # Unique filename
json_file = os.path.join(WEIGHTS_DIR, 'prednet_kitti_5layers_nopool_model_Lall.json') # Unique filename

# Data files
train_file = os.path.join(DATA_DIR, 'X_train.hkl')
train_sources = os.path.join(DATA_DIR, 'sources_train.hkl')
val_file = os.path.join(DATA_DIR, 'X_val.hkl')
val_sources = os.path.join(DATA_DIR, 'sources_val.hkl')

# Training parameters (same as original)
nb_epoch = 150
batch_size = 4
samples_per_epoch = 500
N_seq_val = 100
nt = 10

# --- Model parameters for 5 Layers ---
n_channels, im_height, im_width = (3, 128, 160)
input_shape = (n_channels, im_height, im_width) if K.image_data_format() == 'channels_first' else (im_height, im_width, n_channels)
# Added one more layer with 256 channels (adjust if needed)
stack_sizes = (n_channels, 48, 96, 192, 256)
R_stack_sizes = stack_sizes
A_filt_sizes = (3, 3, 3, 3)          # Length: num_layers - 1 = 4
Ahat_filt_sizes = (3, 3, 3, 3, 3)     # Length: num_layers = 5
R_filt_sizes = (3, 3, 3, 3, 3)        # Length: num_layers = 5
layer_loss_weights = np.array([1., 0.1, 0.1, 0.1, 0.1]) # Lall
layer_loss_weights = np.expand_dims(layer_loss_weights, 1)
# --- End Model parameters ---

time_loss_weights = 1./ (nt - 1) * np.ones((nt,1))
time_loss_weights[0] = 0

prednet = PredNet(stack_sizes, R_stack_sizes,
                  A_filt_sizes, Ahat_filt_sizes, R_filt_sizes,
                  output_mode='error', return_sequences=True,
                  data_format=K.image_data_format())

inputs = Input(shape=(nt,) + input_shape)
errors = prednet(inputs)
errors_by_time = TimeDistributed(Dense(1, trainable=False), weights=[layer_loss_weights, np.zeros(1)], trainable=False)(errors)
errors_by_time = Flatten()(errors_by_time)
final_errors = Dense(1, weights=[time_loss_weights, np.zeros(1)], trainable=False)(errors_by_time)
model = Model(inputs=inputs, outputs=final_errors)

optimizer = 'adam'
model.compile(loss='mean_absolute_error', optimizer=optimizer)

train_generator = SequenceGenerator(train_file, train_sources, nt, batch_size=batch_size, shuffle=True, data_format=K.image_data_format())
val_generator = SequenceGenerator(val_file, val_sources, nt, batch_size=batch_size, N_seq=N_seq_val, data_format=K.image_data_format())

lr_schedule = lambda epoch: 0.001 if epoch < 75 else 0.0001
callbacks = [LearningRateScheduler(lr_schedule)]
if save_model:
    if not os.path.exists(WEIGHTS_DIR): os.makedirs(WEIGHTS_DIR)
    callbacks.append(ModelCheckpoint(filepath=weights_file, monitor='val_loss', save_best_only=True))

history = model.fit_generator(train_generator, steps_per_epoch = samples_per_epoch // batch_size, epochs = nb_epoch, callbacks=callbacks,
                validation_data=val_generator, validation_steps = N_seq_val // batch_size)

if save_model:
    json_string = model.to_json()
    with open(json_file, "w") as f:
        f.write(json_string)

print("Training complete for 5 layers. Weights saved to: {}".format(weights_file))

