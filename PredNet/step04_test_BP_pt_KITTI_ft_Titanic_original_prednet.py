...
BP_Results_DIR = '/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/08_Budapest/result'
for run in runs:
    print("\nProcessing run: {}".format(run))
    # --- Load Test Data ---
    test_file = os.path.join(BP_HICKLE_DUMP, 'X_{}.hkl'.format(run))
    test_sources = os.path.join(BP_HICKLE_DUMP, 'sources_{}.hkl'.format(run))
    ...
    for output_mode in layers4_output_modes:
        ...
        save_dir= os.path.join('{}/prednet_original/npy/pt_kitti_ft_Lall_nt150/layer4/{}'.format(BP_Results_DIR, run))
        print(save_dir)
