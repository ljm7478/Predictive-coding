...
for model in model_type[1:2]: 
    for layer in layer_type[1:2]: 
        if layer == 'layer4':
            num_of_features = 4 
        else:
            num_of_features = 5
        hdf5_dir=os.path.join('/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/01_gump/result/{}/h5/pt_kitti_ft_Lall_nt150/{}'.format(model, layer))
        if not os.path.exists(hdf5_dir):
            os.makedirs(hdf5_dir)
        for run in run_names[4:8]:
            npy_dir = os.path.join('/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/01_gump/result/{}/npy/pt_kitti_ft_Lall_nt150/{}/run{}').format(model, layer, int(run[-1])+1)
            print(npy_dir)
...
# model_type = ['prednet_original', 'CNN_like']
# layer_type = ['layer4', 'layer5_nopool']
# run_names = ['seg1', 'seg2', 'seg3', 'seg4', 'seg5'] 
...
#         else:
#             num_of_features = 5
#         hdf5_dir=os.path.join('/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/08_Budapest/result/{}/h5/pt_kitti_ft_Lall_nt150/{}'.format(model, layer))
#         if not os.path.exists(hdf5_dir):
#             os.makedirs(hdf5_dir)
#         for run in run_names:
#             npy_dir = os.path.join('/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/08_Budapest/result/{}/npy/pt_kitti_ft_Lall_nt150/{}/run{}').format(model, layer, int(run[-1]))
#             print(npy_dir)
#             for feature in features:
#                 hdf5_file_path = os.path.join(hdf5_dir, '{}_{}.h5'.format(run, feature))
#                 with h5py.File(hdf5_file_path, 'w') as hf:
