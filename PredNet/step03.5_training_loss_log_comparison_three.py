...
path_concat6movies = "/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/02_Titanic/weights/prednet/pt_concat6movies_ft_Lall_nt150/layer4/fix_hyperparameter_midsucess/history.csv"
...
try:
    ...
    df2 = pd.read_csv(path_concat6movies)
    df3 = pd.read_csv(path_kitti)
except IOError as e:
...
ax1.set_title('500SUM+CIT Finetune')
...
# --- Plot 2: Final ---
ax2.plot(df2['epoch'], df2['loss'], label='Training Loss')
ax2.plot(df2['epoch'], df2['val_loss'], label='Validation Loss')
ax2.set_title('Concat6Movies Finetune')
...
# --- Plot 3: Final 2 ---
ax3.plot(df3['epoch'], df3['loss'], label='Training Loss')
ax3.plot(df3['epoch'], df3['val_loss'], label='Validation Loss')
ax3.set_title('KITTI Finetune')
...
# --- 6. Finalize and Save ---
fig.suptitle('Training Log Comparison (3 Versions)', fontsize=16)
