...
caffe_root = '/combinelab2/03_user/jungmin/02_data/14_DNN_encoding_pretrained/caffe-model-zoo/caffe_2/caffe/'
data_root = '/combinelab2/03_user/jungmin/02_data/03_NeuralEncodingDecoding/stimuli/video_fmri_dataset/'
output_dir = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/AlexNet/feature_maps/'
...
for seg in range(1,Ns+1):
    foldpath = output_dir+'AlexNet_feature_maps_seg' + str(seg)+'.h5'
    ...
    for im in numlist:
        ...
        image_path = os.path.join(data_root, 'train', 'seg' + str(seg), 'frames', 'im-' + str(im + 1) + '.jpg')
...
# numlist = np.arange(0,numOfimages,int(30/srate)) # subsamle if needed
# for seg in range(1,Ns+1):
#     foldpath = output_dir+'AlexNet_feature_maps_test' + str(seg)+'.h5'
#     store = h5py.File(foldpath,'w')
#     act={}
#     for lay_idx in range(0,len(layer_name_list)): 
#         layer_name = layer_name_list[lay_idx]
...
#         image = caffe.io.load_image(data_root+'/test/test'+str(seg)+'/frames/im-'+str(im+1)+'.jpg')
#         transformed_image = transformer.preprocess('data', image)
#         net.blobs['data'].data[...] = transformed_image
#         output = net.forward()
#         for lay_idx in range(0,len(layer_name_list)): 
