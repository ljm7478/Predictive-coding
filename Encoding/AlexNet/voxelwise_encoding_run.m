...
dataroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/AlexNet/feature_maps/';
saveroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/AlexNet/encoding/';
% 
...
for lay = 1 : length(layername)
    ...
    h5create([saveroot,'AlexNet_feature_maps_avg.h5'],[layername{lay},'/data'],...
        [size(lay_feat_mean)],'Datatype','single');
    h5write([saveroot,'AlexNet_feature_maps_avg.h5'], [layername{lay},'/data'], lay_feat_mean);
end

% calculate the temporal standard deviation
for lay = 1 : length(layername)
    lay_feat_mean = h5read([saveroot,'AlexNet_feature_maps_avg.h5'], [layername{lay},'/data']);
    N = 0;
    for seg = 1 : 18
        disp(['Layer: ',layername{lay},'; Seg: ', num2str(seg)]);
        secpath = [dataroot,'AlexNet_feature_maps_seg', num2str(seg),'.h5'];
    ...
    h5create([saveroot,'AlexNet_feature_maps_std.h5'],[layername{lay},'/data'],...
        [size(lay_feat_mean)],'Datatype','single');
    h5write([saveroot,'AlexNet_feature_maps_std.h5'], [layername{lay},'/data'], lay_feat_std);
end
...
for lay = 1 : length(layername)
    lay_feat_mean = h5read([saveroot,'AlexNet_feature_maps_avg.h5'], [layername{lay},'/data']);
    lay_feat_std = h5read([saveroot,'AlexNet_feature_maps_std.h5'], [layername{lay},'/data']);
...
%         Nf = size(lay_feat,2);
    ...
    save([saveroot,'AlexNet_feature_maps_pca_layer',num2str(lay),'.mat'], 'B', 's', '-v7.3');
end
...
for lay = 1 : length(layername)
    ...
    % save principal components
    save([saveroot,'AlexNet_feature_maps_svd_layer',num2str(lay),'.mat'], 'B', 's', '-v7.3');
end
...
for lay = 1 : length(layername)
    lay_feat_mean = h5read([saveroot,'AlexNet_feature_maps_avg.h5'], [layername{lay},'/data']);
    lay_feat_std = h5read([saveroot,'AlexNet_feature_maps_std.h5'], [layername{lay},'/data']);
    load([saveroot,'AlexNet_feature_maps_svd_layer',num2str(lay),'.mat'], 'B');
    ...
    for seg = 1 : 18
        ...
        if exist(secpath,'file')==2
            ...
            h5create([saveroot,'AlexNet_feature_maps_pcareduced_seg', num2str(seg),'.h5'],[layername{lay},'/data'],...
                [size(ts)],'Datatype','single');
            h5write([saveroot,'AlexNet_feature_maps_pcareduced_seg', num2str(seg),'.h5'], [layername{lay},'/data'], ts);
        end
    end   
    ...
    for seg = 1 : 5
        ...
        if exist(secpath,'file')==2
            ...
            h5create([saveroot,'AlexNet_feature_maps_pcareduced_test', num2str(seg),'.h5'],[layername{lay},'/data'],...
                [size(ts)],'Datatype','single');
            h5write([saveroot,'AlexNet_feature_maps_pcareduced_test', num2str(seg),'.h5'], [layername{lay},'/data'], ts);
        end
    end  
...
for lay = 1 : length(layername)
    for seg = 1 : 18
        disp(['Layer: ', num2str(lay),'; Seg: ',num2str(seg)]);
        secpath = [saveroot,'AlexNet_feature_maps_pcareduced_seg', num2str(seg),'.h5'];      
    ...
    h5create([saveroot,'AlexNet_feature_maps_pcareduced_concatenated.h5'],[layername{lay},'/data'],...
        [size(Y)],'Datatype','single');
    h5write([saveroot,'AlexNet_feature_maps_pcareduced_concatenated.h5'], [layername{lay},'/data'], Y);
end
