...
addpath(fullfile('/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/code/PredNet/Nunez_test/helpers'));
%% dir 
basepath = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/Nunez_test/';
groupdir = fullfile(basepath, ['results/group/' model '/']);
if ~exist(groupdir, 'dir')
    mkdir(groupdir);
end
%% load fMRI 
basepath = '/combinelab/03_user/jungmin/02_data/08_NeuralEncodingDecoding/';
fmripath = fullfile(basepath, 'subject1','/video_fmri_dataset/', 'subject1', '/fmri/','seg1', '/cifti/');
filename = 'seg1_1_Atlas.dtseries.nii';

%% save fMRI mat for cosmo_fmri_dataset
basepath = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/';
fmripath = fullfile(basepath, 'subject1_fmri','/');
filename = 'training_fmri.mat';
...
samples = (sub1.data1+sub1.data2) / 2;
samples = reshape(samples, size(samples,1), size(samples,2)*size(samples,3));

featuredir = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/Nunez_test/';
save([featuredir,'sub1_', filename],'samples') ; 
%% load fmri 
fmripath = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/Nunez_test/';
filename='sub1_training_fmri.mat';
%% load feature mat
clc;
name_feature={'/Ahat1';'/Ahat2';'/Ahat3';'/Ahat4'}; 
featuredir = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/Nunez_test/';
feature1=load(fullfile(featuredir,'Ahat1.mat')) %load your feature space ; a feature space that use the same lambda
feature2=load(fullfile(featuredir,'Ahat2.mat')) %load another feature space (to use banded ridge, you may want to provide two or more feature spaces).
nfeature=size(name_feature,2);
X1=normalize(feature1.lay_feat_concatenated); 
X2=normalize(feature2.lay_feat_concatenated);
...
for subj=1
    ...
    clear data_clean;
    savedir = fullfile(basepath, ['results/sub' num2str(subj) '/' model '/']);
