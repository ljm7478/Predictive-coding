...
for lay = 1: length(layername)
    ...
    for stim = 1:length(stimulus_type)
        ...
        % Path to the H5 file
        dataroot = fullfile(base_dir, stimulus_type{stim}, session_type, 'result/prednet_original/h5/', layertype(1:6));
...
for session_idx = 1:size(sessions, 1)
    ...
    for lay = 1: length(layername)
        ...
        for stim = 1:length(curr_stimulus_type)
            ...
            dataroot = fullfile(base_dir, curr_stimulus_type{stim}, curr_session_type, 'result/prednet_original/h5/', layertype(1:6));
            secpath = fullfile(dataroot, 'Ahat.h5');
