...
feature_path = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/Jmlee_test/encoding_analysis/modified_prednet/concat_time/Ahat/garbor';
...
save_path = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/Jmlee_test/encoding_analysis/modified_prednet/concat_time/Ahat/garbor';
...
for config_idx = 1:size(configs, 1)
    ...
    for lay = 1:length(layername)
        ...
        for stim = fieldnames(stimulus_mapping)'
            ...
            if contains(lower(task_name), 'Unpred')
                group_type = 'pred';
            else
                group_type = 'Unpred';
            end
            % Construct feature and fMRI file paths
            feature_file = fullfile(feature_path, ['PredNet_feature_maps_processed_', stim_key, '_layer', num2str(lay), '.h5']);
            ...
            if exist(feature_file, 'file') && exist(fmri_file, 'file')
                ...
                load(fmri_file, 'fmri_train_unpred');  % Load fMRI data
                ...
                fmri_train_avg = mean(fmri_train_unpred, 3);  % Average across 3rd dimension
            else
    ...
    for lay = 1:length(layername)
        ...
        for stim_name = fieldnames(stimulus_mapping)'
            ...
            if exist(feature_file, 'file') && exist(fmri_file, 'file')
                ...
                load(fmri_file, 'fmri_test_unpred');

                % Concatenate data
                Y_test = [Y_test; Y_data];
                fmri_test = cat(3, fmri_test, fmri_test_unpred);
            else
        ...
        % Unpredict fMRI responses
        fmri_test_avg = mean(fmri_test, 3);
        Unpred_fmri = Y_test * W;

        % Calculate correlations
        fmri_test_avg = fmri_test_avg';
        correlation_scores = arrayfun(@(v) corr(Unpred_fmri(:, v), fmri_test_avg(:, v)), 1:size(Unpred_fmri, 2));

        % Save correlations
        corr_file = fullfile(save_path, ['corr_lambda_', test_date(1:6), '_', task_name(7:end),'_layer', num2str(lay), '.mat']);
...
corr_path = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/UnpredNet/Titanic_trained/Jmlee_test/encoding_analysis/modified_prednet/concat_time/Ahat/';
fmripath = '/combinelab2/03_user/jungmin/02_data/02_jmlee_fmri/02_fmri/Preprocessed/241119_noT1/output/ciftify/sub-01/MNINonLinear/Results/task-CircleUnpred_run-01/task-CircleUnpred_run-01_Atlas_s3.dtseries.nii';
...
for lay = 1:length(layername)
    ...
    for config_idx = 1:size(configs, 1)
        test_date = configs{config_idx, 2};
        corr_file = fullfile(corr_path, ['corr_lambda_', test_date(1:6), '_Unpred_layer', num2str(lay), '.mat']);
...
save_file_name = fullfile(corr_path, 'lambda5_E_avg_corr.dtseries.nii');
...
feature_path = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/Jmlee_test/encoding_analysis/modified_prednet/concat_time/Ahat/movie';
...
% Stimulus groups (excluding movie tasks)
stimulus_mapping = struct('movie', 'MovieUnpred');
...
clc;
% Paths for CIFTI and correlation files
corr_path = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/UnpredNet/Titanic_trained/Jmlee_test/encoding_analysis/modified_prednet/concat_time/Ahat/movie/';
fmripath = '/combinelab2/03_user/jungmin/02_data/02_jmlee_fmri/02_fmri/Preprocessed/241119_noT1/output/ciftify/sub-01/MNINonLinear/Results/task-CircleUnpred_run-01/task-CircleUnpred_run-01_Atlas_s3.dtseries.nii';
