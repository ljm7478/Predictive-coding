"""
Step 3: GPU-accelerated Voxel-wise Encoding for Budapest Dataset
...
BASE_SAVE_DIR = "/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/Budapest_test/encoding_analysis/original_prednet/kitti_pretrain_result/four_train_four_test/layer4"
...
# Budapest data split
# Regressors: run1.h5, run2.h5, run3.h5, run4.h5, run5.h5
# fMRI: run1, run2, run3, run4, run5
train_runs = [1, 2, 3]   # seg1, seg2, seg3  598 + 498 + 535 = 1631 TRs
...
# Budapest subject IDs (actual naming from your preprocessing script)
subjects = [
...
def run_budapest_encoding():
    """Main encoding pipeline for Budapest dataset."""
    
    alphas = np.array(lambdas, dtype=np.float32)
    total_start = time.time()
    
    print(f"\n{'='*70}")
    print(f" BUDAPEST ENCODING: {FEATURE_TYPE}")
    print(f"{'='*70}")
...
def quick_shape_check():
    ...
    print("\n" + "="*70)
    print(f" QUICK SHAPE CHECK - Budapest Dataset ({FEATURE_TYPE})")
    print("="*70)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Budapest Encoding Pipeline")
