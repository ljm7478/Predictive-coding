...
saveroot = ['/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/encoding_analysis/modified_prednet/layers5_3_32_64_96_192channel/test1/test1_E_event_time/ver2_', subject_id, '/'];
...
for lay = 1:num_layers
    ...
    for idx = 1:num_events
        ...
        if event_idx + 4 <= size(pred_fmri, 1)
            mean_responses(idx, :) = mean(pred_fmri(event_idx+2:event_idx+4, :), 1);
        end
    end
...
save_file_name = fullfile(saveroot, [subject_id, '_lambda5_E_rev2.dtseries.nii']);
