...
function [W, Rmat, Lambda] = voxelwise_encoding(Y, X, lambda, nfold)
    ...
    % jmlee: Check if dT is not an integer - happens when Nc is odd
    if mod(dT, 1) ~= 0
        % If dT is a float, round to the nearest integer
        dT = floor(dT);
    end
    ...
    disp('Training encoidng models with optimal regularskization parameters ..');
    YTY = Y'*Y;
