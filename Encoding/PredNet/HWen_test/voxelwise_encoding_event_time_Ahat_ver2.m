...
saveroot = ['/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/encoding_analysis/modified_prednet/layers5_3_32_64_96_192channel/test1/test1_Ahat_event_time/ver2_', subject_id, '/'];
...
%% Compute mean of i+2, i+3, i+4 TRs for each event to create the surprise time series - pred fMRI (Y)
surprise_time_series = cell(num_layers, 1);
for lay = 1:num_layers
    pred_fmri = all_pred_fmri{lay};
    num_events = length(adjusted_event_indices);
    mean_responses = zeros(num_events, size(pred_fmri, 2));
    
    for idx = 1:num_events
        event_idx = adjusted_event_indices(idx);
        if event_idx + 4 <= size(pred_fmri, 1)
            mean_responses(idx, :) = mean(pred_fmri(event_idx:event_idx+2, :), 1);
        end
    end
    
    surprise_time_series{lay} = mean_responses;
end


%% Compute mean of i+2, i+3, i+4 TRs for each event to create the surprise time series - actual fMRI (X)

num_events = length(adjusted_event_indices);
mean_responses = zeros(num_events, size(extracted_fmritest1_avg, 2));

for idx = 1:num_events
    event_idx = adjusted_event_indices(idx);
    if event_idx + 4 <= size(extracted_fmritest1_avg, 1)
        mean_responses(idx, :) = mean(extracted_fmritest1_avg(event_idx:event_idx+2, :), 1);
    end
end

actual_BOLD_surprise_time = mean_responses;

%% Correlate extracted signals
all_corr = cell(num_layers, 1);
for lay = 1:num_layers
    pred_fmri_events = surprise_time_series{lay};  
    correlation_scores = zeros(1, size(pred_fmri_events, 2));
    
    for v = 1:size(pred_fmri_events, 2)
        correlation_scores(v) = corr(pred_fmri_events(:, v), actual_BOLD_surprise_time(:, v));
    end
...
save_file_name = fullfile(saveroot, [subject_id, '_lambda5_Ahat_rev2.dtseries.nii']);
ciftisavereset(cii, save_file_name, 'wb_command');

disp(['Saved results to: ', save_file_name]);
%% Save the surprise time series
for lay = 1:num_layers
    file_name = fullfile(saveroot, [subject_id, '_surprise_timeseries_', layername{lay}(2:end), '.mat']);
    save(file_name, 'surprise_time_series', '-v7.3');
end

disp('Surprise time series has been computed and saved.');
