...
%first trial fmri
fmritest1_1st = fmritest.test1(:,:,1);
...
saveroot = ['/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/encoding_analysis/modified_prednet/layers5_3_32_64_96_192channel/test1/test1_E_event_time/', subject_id, '/'];
...
% saveroot = ['/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/encoding_analysis/modified_prednet/layers5_96_256_384_384_256channel/test1/test1_E_event_time/', subject_id, '/'];
...
extracted_fmritest1_avg = fmritest1_1st (:, adjusted_event_indices)';
...
save_file_name = fullfile(saveroot, [subject_id, '_lambda5_E_rev_first_trial.dtseries.nii']);
ciftisavereset(cii, save_file_name, 'wb_command');

disp(['Saved results to: ', save_file_name]);

%% Compare predicted BOLD vd fMRI BOLD
clc;
%%%%%%%%%%%%%%%% fMRIBOLD
% Load CIFTI structure 
subject_id = 'subject1';
...
% Save back into dtseries for visualization
cii.cdata = extracted_fmritest1_avg';
cii.diminfo{1, 1}.length = size(cii.cdata, 1);
cii.diminfo{1, 2}.length = size(cii.cdata, 2);
cii.diminfo{1, 1}.models(3:end) = [];
save_file_name = fullfile(saveroot, [subject_id, '_event_time_fMRI_BOLD_first_trial.dtseries.nii']);
ciftisavereset(cii, save_file_name, 'wb_command');
...
pred_fmri_events = cell(num_layers, 1);
for lay = 1:num_layers
    pred_fmri_events{lay} = extracted_pred_fmri{lay,1};
end

% Save back into dtseries for visualization
cii.cdata = pred_fmri_events{1}';
cii.diminfo{1, 1}.length = size(cii.cdata, 1);
cii.diminfo{1, 2}.length = size(cii.cdata, 2);
cii.diminfo{1, 1}.models(3:end) = [];
save_file_name = fullfile(saveroot, [subject_id, '_event_time_E1_predicted_BOLD_first_trial.dtseries.nii']);
ciftisavereset(cii, save_file_name, 'wb_command');
