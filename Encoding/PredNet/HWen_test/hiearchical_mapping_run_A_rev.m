...
%% Concatenate model activation time series in the 1st layer across movie segments
layername = {'/A1';'/A2';'/A3';'/A4';}; 

dataroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/bivariate_analysis/layers5_32_64_96_193channel/A/';
saveroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/hiearchical_mapping/layers5_32_64_96_193channel/A';

% Training movies
for lay = 1 : length(layername)
    for seg = 1 : 18
        disp(['Seg: ', num2str(seg)]);
        secpath = [dataroot,'PredNet_feature_maps_processed_seg', num2str(seg),'_A', '.h5'];      
        % info = h5info(secpath);
    ...
    save([saveroot,'PredNet_feature_maps_processed_layer',num2str(lay),'_A', '_concatenated.mat'],...
...
% dataroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/hiearchical_mapping/layers5_3_32_64_96_192channel/A/';
saveroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/hiearchical_mapping/modified_prednet/layers5_3_32_64_96_192channel/A/';
...
for sub_idx = 1:length(subjects)
    subject_id = subjects{sub_idx};
    sub_short_id = ['sub', subject_id(end)]; % Create 'sub1', 'sub2', etc.

    fmriroot = sprintf('/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/%s_fmri/', subject_id);

    % Load fMRI responses
    load([fmriroot, 'training_fmri.mat'], 'fmri'); % from movie_fmri_processing.m
    ...
    % Map hierarchical CNN features to brain - whole voxels
    voxels = 1:59412;
    lay_corr = zeros(length(layername), length(voxels));

    for lay = 1:length(layername)
        disp(['Mapping CNN features to brain, Subject: ', sub_short_id, ', Layer: ', num2str(lay)]);
        load([saveroot, 'PredNet_feature_maps_processed_layer', num2str(lay), '_A_concatenated.mat'], 'lay_feat_concatenated');
        ...
        filename = sprintf('jmlee_%s_cross_corr_fmri_cnn_layer%s_A.mat', sub_short_id, num2str(lay)); 
        save([saveroot,filename], 'Rmat', '-v7.3');

        lay_corr(lay, :) = max(Rmat, [], 1);
    end
    
    filename = sprintf('jmlee_%s_cross_max_corr_fmri_cnn_A.mat', sub_short_id); 
    save([saveroot, filename], 'lay_corr');
    [~, layidx] = max(lay_corr, [], 1);
    filename = sprintf('jmlee_%s_cross_max_corr_layidx_fmri_cnn_A.mat', sub_short_id); 
    save([saveroot, filename], 'layidx');
end
...
for sub_idx = 1:length(subjects)
    ...
    % Load subject cii for visualization
    fmripath = sprintf('/combinelab/03_user/jungmin/02_data/08_NeuralEncodingDecoding/%s/video_fmri_dataset/%s/fmri/', subject_id, subject_id);
    seg = 1;
    filename = ['seg', num2str(seg), '/cifti/seg', num2str(seg), '_1_Atlas.dtseries.nii'];
    cii = ciftiopen([fmripath, filename], 'wb_command');

    % Save max correlation and layer index for whole brain
    filename = sprintf('jmlee_%s_cross_max_corr_fmri_cnn_A.mat', sub_short_id); 
    load([saveroot, filename], 'lay_corr');
    cii.cdata = lay_corr';
    cii.diminfo{1, 1}.length = size(cii.cdata, 1);
    cii.diminfo{1, 2}.length = size(cii.cdata, 2);
    cii.diminfo{1, 1}.models(3:end) = [];
    save_file_name = fullfile(saveroot, [sub_short_id, '_WH_max_laycorr_A.dtseries.nii']);
    ciftisavereset(cii, save_file_name, 'wb_command');

    filename = sprintf('jmlee_%s_cross_max_corr_layidx_fmri_cnn_A.mat', sub_short_id); 
    load([saveroot, filename], 'layidx');
    cii.cdata = layidx';
    cii.diminfo{1, 1}.length = size(cii.cdata, 1);
    cii.diminfo{1, 2}.length = size(cii.cdata, 2);
    cii.diminfo{1, 1}.models(3:end) = [];
    save_file_name = fullfile(saveroot, [sub_short_id, '_WH_max_layidx_A.dtseries.nii']);
    ciftisavereset(cii, save_file_name, 'wb_command');
end
...
%layer max correlation
for sub_idx = 1:length(subjects)
    sub_short_id = ['sub', subject_id(end)]; % Create 'sub1', 'sub2', etc.

    filename = sprintf('jmlee_%s_cross_max_corr_fmri_cnn_A.mat', sub_short_id);
    load([saveroot, 'jmlee_sub1_cross_max_corr_fmri_cnn_A.mat'], 'lay_corr');
    ...
    cii.diminfo{1, 2}.length = size(cii.cdata, 2);
    cii.diminfo{1, 1}.models(3:end) = [];
    save_file_name = fullfile(saveroot, [sub_short_id, '_visual_max_laycorr_A.dtseries.nii']);
    ciftisavereset(cii, save_file_name, 'wb_command');
end
...
for sub_idx = 1:length(subjects)
    ...
    cii.diminfo{1, 2}.length = size(cii.cdata, 2);
    cii.diminfo{1, 1}.models(3:end) = [];
    save_file_name = fullfile(saveroot, [sub_short_id, '_visual_max_layidx_A.dtseries.nii']);
    ciftisavereset(cii, save_file_name, 'wb_command');
end 

%% Compare max_lay_idx in visual only between prediction vs error
prediction = load([dataroot, 'Ahat/jmlee_cross_max_corr_layidx_fmri_cnn_Ahat.mat'], 'layidx').layidx;
error = load([dataroot, 'E/jmlee_sub1_cross_max_corr_layidx_fmri_cnn_E.mat'], 'layidx').layidx;

concat_pred_error = cat(1, prediction, error); % first row-prediction; second row-error

for lay = 1:2
    disp(['Layer: ', num2str(lay)]);
    [~, layidx] = max(concat_pred_error, [], 1); % Assign max layer; 1-prediction, 2-error
end
...
cii.diminfo{1, 2}.length = size(cii.cdata, 2);
cii.diminfo{1, 1}.models(3:end) = [];
save_file_name = fullfile(saveroot, [sub_short_id, '_visual_max_Ahat_vs_Error.dtseries.nii']);
ciftisavereset(cii, save_file_name, 'wb_command');
