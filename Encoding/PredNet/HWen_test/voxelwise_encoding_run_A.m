...
dataroot='/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/encoding_analysis/A/';
saveroot = ['/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/encoding_analysis/A/', subject_id, '/'];
...
save_file_name = fullfile(saveroot, [subject_id,'_lambda5_A.dtseries.nii']);
...
cii.diminfo{1, 2}.length = size(cii.cdata, 2);
cii.diminfo{1, 1}.models(3:end) = [];
save_file_name = fullfile(saveroot, [subject_id,'_visual_lambda5_A.dtseries.nii']);
ciftisavereset(cii, (save_file_name), 'wb_command');

%% total sub encoding 
clc;clear 
saveroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/encoding_analysis/A/';
layername = {'/A1', '/A2', '/A3', '/A4'};  
...
for lay = 1:num_layers
    ...
    for s = 1:num_subjects
        ...
        dataroot = ['/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/AlexNet_test/encoding_analysis/A/', subject_id, '/'];
        file_name = [subject_id, '_visual_corr_lambda5_', layername{lay}(2:end), '.mat'];
...
corr_avg = load([saveroot, 'A_avg_corr.mat']).corr_avg;
...
cii.diminfo{1, 2}.length = size(cii.cdata, 2);
cii.diminfo{1, 1}.models(3:end) = [];
save_file_name = fullfile(saveroot, 'total_sub_lambda5_A.dtseries.nii');
ciftisavereset(cii, (save_file_name), 'wb_command');
...
corr_avg = load([saveroot, 'A_visual_avg_corr.mat']).corr_avg;
corr_avg = cat(2, corr_avg{1,1}, corr_avg{2,1}, corr_avg{3,1}, corr_avg{4,1});
...
cii.diminfo{1, 2}.length = size(cii.cdata, 2);
cii.diminfo{1, 1}.models(3:end) = [];
save_file_name = fullfile(saveroot, 'visual_sub_lambda5_A.dtseries.nii');
ciftisavereset(cii, (save_file_name), 'wb_command');
...
% lay1=load([saveroot, 'corrA1.mat']).R;
% lay2=load([saveroot, 'corrA2.mat']).R;
% lay3=load([saveroot, 'corrA3.mat']).R;
% lay4=load([saveroot, 'corrA4.mat']).R;

lay1=load([saveroot, 'corr_avg_visual_A1.mat']).R;
lay2=load([saveroot, 'corr_avg_visual_A1.mat']).R;
lay3=load([saveroot, 'corr_avg_visual_A3.mat']).R;
lay4=load([saveroot, 'corr_avg_visual_A4.mat']).R;
