...
SAVE_ROOT = fullfile('/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/GUMP_test/encoding_analysis/original_prednet/kitti_pretrain_result/longer_time/four_train_four_test/layer4/', FEATURE_TYPE, 'indirect_10s_extrap');
...
for lay = 1:num_layers
    ...
    for i = 1:length(train_runs)
        ...
        filepath = fullfile(FEATURE_DATA_ROOT, 'indirect_10s_extrap/layer4', ['seg', num2str(run_idx), '_', FEATURE_TYPE, '.h5']);
    ...
    template_dims = size(h5read(fullfile(FEATURE_DATA_ROOT, 'indirect_10s_extrap/layer4', ['seg0_', FEATURE_TYPE, '.h5']), layer_names{lay}));
...
% 
%     subject_save_dir = fullfile(SAVE_ROOT, 'indirect_10s_extrap', subject);
...
%         for i = 1:length(train_runs)
%             run_idx = train_runs(i);
%             regressor_path = fullfile(SAVE_ROOT, 'indirect_10s_extrap', 'regressors', ['run', num2str(run_idx+1), '.h5']);
