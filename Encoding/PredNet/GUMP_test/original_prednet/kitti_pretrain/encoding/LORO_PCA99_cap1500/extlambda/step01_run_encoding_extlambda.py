"""
Full re-run with extended log-spaced λ grid [0.01 … 100] (9 values).
Based on step01_run_encoding_lambda_diag_E.py.

Changes vs. the original:
  - Extended λ grid (4 decades, log-spaced)
  - Output to BASE_SAVE_DIR + "_extlambda" so existing results are preserved
  - Feature type selectable via --feat {Ahat, E}
  - Saves Lambda_layer{N}.npy per subject/layer as before
"""
...
if USE_LOG10:
    BASE_SAVE_DIR = (
        ...
        "kitti_pretrain_result/LORO_PCA99cap1500_log10/layer4_extlambda"
    )
else:
    BASE_SAVE_DIR = (
        "/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/"
        "PredNet/Titanic_trained/GUMP_test/encoding_analysis/original_prednet/"
        "kitti_pretrain_result/LORO_PCA99cap1500/layer4_extlambda"
    )

# Regressors live in the original (non-extlambda) tree
REG_BASE_DIR = BASE_SAVE_DIR.replace("_extlambda", "")
...
def save_cifti_dscalar(data, template, out_path, feature_type):
    ...
    if n_gray != len(cortex_idx):
        print(f"    WARNING: data {n_gray} != cortex {len(cortex_idx)}; skip CIFTI")
        return False
...
def run(feature_type, device):
    ...
    t_total = time.time()
    print(f"\n{'='*70}")
    print(f" EXT-λ ENCODING  feat={feature_type}  {N_FOLDS}-fold LORO")
    ...
    for si, subj in enumerate(SUBJECTS):
        ...
        for L in range(num_layers):
            ...
            print(
                f"      Layer pooled λ: {summarize_hist(Lam_layer.ravel(), LAMBDAS)}"
            )
            print(
        ...
        if len(all_layer_corrs) == num_layers:
            ...
            print(
                ...
                f"mean r across layers={np.nanmean(final):.4f}"
            )
