"""
Extended λ grid + per-feature z-scoring of regressors.

Same as step01_run_encoding_extlambda.py, but each PC is z-scored using
training-fold statistics before ridge. This gives every feature equal
scale so the scalar λ has a consistent meaning across all 1500 PCs.

Rationale: feature_variance_audit.py showed variances spanning 3.3–5.2
decades across PCs, which forces CV to pick extreme λs because a single
scalar can't balance shrinkage across features.

Output: .../LORO_PCA99cap1500_log10/layer4_extlambda_zscore/{Ahat,E}/
"""
...
if USE_LOG10:
    BASE_SAVE_DIR = (
        ...
        f"kitti_pretrain_result/LORO_PCA99cap1500_log10/layer4{_SUFFIX}"
    )
else:
    BASE_SAVE_DIR = (
        "/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/"
        "PredNet/Titanic_trained/GUMP_test/encoding_analysis/original_prednet/"
        f"kitti_pretrain_result/LORO_PCA99cap1500/layer4{_SUFFIX}"
    )

# Regressors live under the original (no-suffix) tree
REG_BASE_DIR = BASE_SAVE_DIR.replace(_SUFFIX, "")
...
def run(feature_type, device):
    ...
    t_total = time.time()
    print(f"\n{'='*70}")
    print(f" EXT-λ + Z-SCORE  feat={feature_type}  {N_FOLDS}-fold LORO")
    ...
    for si, subj in enumerate(SUBJECTS):
        ...
        for L in range(num_layers):
            ...
            for fi, test_run in enumerate(ALL_RUNS):
                ...
                # Per-feature z-score using TRAINING stats only (no leakage)
                mx = X_tr.mean(0, keepdims=True)
                sx = X_tr.std(0, keepdims=True)
                sx = np.where(sx < 1e-8, 1.0, sx)  # guard degenerate features
                X_tr = (X_tr - mx) / sx
                X_te = (X_te - mx) / sx
