"""
Step 1 (lambda-diagnostic variant): GPU-accelerated Voxel-wise Encoding for Gump Dataset
Leave-One-Run-Out (LORO) Cross-Validation Version

This is a copy of step01_run_encoding.py with two additions:
  1. Captures and prints the per-fold histogram of CV-selected ridge λ values,
     and flags pile-ups at the grid boundaries (low / high).
  2. Saves the selected λ per fold per voxel as Lambda_layer{N}.npy in each
     subject's output directory, so the λ choices can be audited later
     without re-running encoding.

The original step01_run_encoding.py is left untouched.
"""
...
def summarize_lambda_histogram(Lambda_arr, alphas):
    """Build a one-line histogram string and a boundary-pile-up flag.

    Lambda_arr : 1D ndarray of CV-selected λ values (one per voxel).
    alphas     : 1D ndarray of the candidate λ grid.
    Returns: (hist_str, flag_str)
    """
    uniq, counts = np.unique(Lambda_arr, return_counts=True)
    pct = counts / counts.sum() * 100
    hist_str = ", ".join(f"{u:.3g}:{p:.1f}%" for u, p in zip(uniq, pct))

    lo, hi = float(alphas.min()), float(alphas.max())
    pct_lo = pct[uniq == lo].sum() if (uniq == lo).any() else 0.0
    pct_hi = pct[uniq == hi].sum() if (uniq == hi).any() else 0.0

    flag = ""
    if pct_lo > 50:
        flag = "  [WARN: λ pile-up at LOW boundary — extend grid downward]"
    elif pct_hi > 50:
        flag = "  [WARN: λ pile-up at HIGH boundary — extend grid upward]"

    return hist_str, flag


# =============================================================================
# 3. MAIN ENCODING LOOP - LORO VERSION (with λ diagnostics)
# =============================================================================

def run_Gump_encoding_LORO():
    """Main encoding pipeline with Leave-One-Run-Out CV and λ diagnostics."""

    alphas = np.array(lambdas, dtype=np.float32)
    total_start = time.time()

    print(f"\n{'='*70}")
    print(f" Gump ENCODING (LORO CV + λ diagnostics): {FEATURE_TYPE}")
    print(f" {n_folds}-fold Cross-Validation")
    print(f" λ grid: {list(alphas)}")
    print(f"{'='*70}")
    ...
    for subj_idx, subject in enumerate(subjects):
        ...
        for lay in range(num_layers):
            ...
            all_fold_Lambda = []  # (n_folds, n_voxels) — selected ridge λ per voxel per fold
            ...
            for fold_idx, test_run in enumerate(all_runs):
                ...
                # Train encoding model — capture Lambda this time
                W, _, Lambda_fold = voxelwise_encoding_new(
                    X_train, fmri_train, alphas, nfold, device=device
                )

                all_fold_W.append(W)
                all_fold_Lambda.append(Lambda_fold)
                ...
                # --- λ diagnostic per fold ---
                hist_str, flag = summarize_lambda_histogram(Lambda_fold, alphas)
                ...
                print(f"        λ histogram: {hist_str}{flag}")
            ...
            # --- Save selected λ per fold per voxel for this layer ---
            Lambda_layer = np.stack(all_fold_Lambda).astype(np.float32)  # (n_folds, n_voxels)
            np.save(subject_save_dir / f"Lambda_layer{lay + 1}.npy", Lambda_layer)

            # Pooled λ summary across all folds for this layer
            pooled_hist, pooled_flag = summarize_lambda_histogram(Lambda_layer.ravel(), alphas)
            print(f"      Layer λ summary (all folds pooled): {pooled_hist}{pooled_flag}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Gump LORO Encoding Pipeline (λ-diagnostic variant)")
