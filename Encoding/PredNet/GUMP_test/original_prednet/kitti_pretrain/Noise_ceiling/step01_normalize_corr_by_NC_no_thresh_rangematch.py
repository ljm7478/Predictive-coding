"""
Create normalized CORRELATION maps WITHOUT threshold + Global Robust Range
- Structure: Single dscalar file per signal type (containing 4 layers)
- Normalization: Raw Correlation / sqrt(ISC)
...
if DATASET == 'budapest':
    ...
    if USE_LOG10:
        ENCODING_DIR = "{}/encoding_analysis/original_prednet/kitti_pretrain_result/LORO_PCAcap_log10/layer4".format(PROJECT_DIR)
    else:
        ENCODING_DIR = "{}/encoding_analysis/original_prednet/kitti_pretrain_result/LORO_PCAcap/layer4".format(PROJECT_DIR)
...
# Output directory (corr-specific)
OUTPUT_DIR = "{}/divergence_analysis/step1_normalized_corr/no_thresh_range_global_merged".format(ENCODING_DIR)
...
ahat_raw = np.load("{}/group_mean_correlations_LORO.npy".format(INPUT_DIR_AHAT))
e_raw = np.load("{}/group_mean_correlations_LORO.npy".format(INPUT_DIR_E))
...
save_and_palette(ahat_norm, "ahat_corr_norm_global", global_max)
save_and_palette(e_norm, "e_corr_norm_global", global_max)
save_and_palette(contrast_norm, "contrast_corr_norm_global", global_max)
