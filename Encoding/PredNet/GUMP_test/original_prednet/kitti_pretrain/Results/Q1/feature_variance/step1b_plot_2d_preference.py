...
# Default ROI mask: data-driven Glasser parcels from step03
ROI_MASK_DEFAULT = (
    "/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/"
    "Titanic_trained/GUMP_test/encoding_analysis/original_prednet/"
    "kitti_pretrain_result/LORO_PCAcap_log10/layer4/Results/Q1/"
    "sig_mask_datadriven/ROI_mask_datadriven.dscalar.nii"
)
...
def make_bivariate_colormap(n_bins=N_BINS_DEFAULT, scheme='red_blue'):
    ...
    Schemes:
        'popham'        : Matches Popham et al. 2021 Fig 2 colors.
                          Ahat = warm red, E = cool cyan/blue, both = white, neither = black.
                          Bilinear interpolation between four corners.
                          Best readability for publication figures.
    ...
    if scheme == 'popham':
        # Four corners matching Popham et al. 2021 Figure 2:
        #   (0,0) = black   (neither)
        #   (1,0) = red     (Ahat only)
        #   (0,1) = blue    (E only)
        #   (1,1) = white   (both)
        ...
        c_red   = np.array([0.85, 0.10, 0.05])   # warm red
        c_blue  = np.array([0.05, 0.45, 0.85])   # cool teal-blue
        c_white = np.array([1.0,  1.0,  1.0])    # white
        for a in range(n_bins):
            for e in range(n_bins):
                fa = (a + 0.5) / n_bins   # fraction Ahat [0,1]
                fe = (e + 0.5) / n_bins   # fraction E    [0,1]
                ...
                idx = a * n_bins + e
                colors[idx] = [float(np.clip(c[0], 0, 1)),
...
def load_roi_mask(mask_path):
    """
    Load a cortical ROI mask CIFTI dscalar and expand to fsLR_32k per-hemi arrays.

    Returns (mask_l, mask_r), each (32492,) boolean, True = inside ROI.
    Non-ROI vertices (label == 0 or NaN) are False.
    """
    mask_img = nib.load(str(mask_path))
    mask_l, mask_r = grayordinates_to_full_surface(mask_img)
    return np.nan_to_num(mask_l[0], nan=0.0) > 0, np.nan_to_num(mask_r[0], nan=0.0) > 0
...
def make_figure(
    ...
    for lay in range(NUM_LAYERS):
        ...
        # Apply ROI mask: non-ROI voxels → bin 0 (darkest color)
...
def make_figure_for_subject(subject, vmax, n_bins, scheme, mode, mask_path=None):
    ...
    ml, mr = (None, None)
    if mask_path:
        ml, mr = load_roi_mask(mask_path)

    _, _, x_label, y_label, mode_caption = MODE_FILES[mode]
    mask_tag = "_ROI" if mask_path else ""

    out_path = OUT_DIR / f"{subject}_2d_preference_{mode}_{scheme}_vmax{vmax:g}{mask_tag}.png"
    title = (
        f"{subject}: {mode_caption}  |  LORO  |  scheme={scheme}  vmax={vmax}"
        + ("  |  ROI masked" if mask_path else "")
    )
    make_figure(
        a_l, a_r, e_l, e_r,
        sulc_l, sulc_r,
...
def make_group_figure(subjects, vmax, n_bins, scheme, mode, mask_path=None):
    """
    ...
    out_path = OUT_DIR / f"GROUP_n{len(subjects)}_2d_preference_{mode}_{scheme}_vmax{vmax:g}{mask_tag}.png"
    title = (
...
def main():
    ...
    parser.add_argument("--scheme", type=str, default="popham",
                        choices=["popham", "red_blue", "PU_RdBu_covar"],
                        help="Bivariate colormap scheme (default: popham)")
    ...
    parser.add_argument("--mask", type=str, default=None,
                        help="Path to ROI mask CIFTI dscalar (non-zero = inside ROI). "
                             "Voxels outside the ROI are rendered as dark (bin 0). "
                             "Use --mask default to use the data-driven Glasser ROI mask.")
    args = parser.parse_args()

    # Resolve mask path
    mask_path = None
    if args.mask == "default":
        mask_path = ROI_MASK_DEFAULT
    ...
    if args.group:
        make_group_figure(args.subjects, vmax=args.vmax, n_bins=args.n_bins,
                          scheme=args.scheme, mode=args.mode, mask_path=mask_path)
    else:
