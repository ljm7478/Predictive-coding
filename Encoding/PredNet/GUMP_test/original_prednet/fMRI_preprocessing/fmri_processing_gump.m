...
for subj = 1:length(subjects)
    ...
    for run = 1:8
        ...
        sigma = std(data, [], 2); % Compute standard deviation per voxel (across time)
        sigma(sigma == 0) = 1; % Prevent divide-by-zero for flat voxels
        data = bsxfun(@rdivide, data, sigma); % Apply voxelwise z-scoring
        % data = bsxfun(@rdivide, data, std(data, [], 2));

        % check if the voxel has NaN 
        if ~any(isnan(data(:)))
            % Store the data for this run
            fmri_data.(['run', num2str(run)]) = data;
        else
            fprintf('Run %d has NaNs skipping storage.\n', run);
        end
