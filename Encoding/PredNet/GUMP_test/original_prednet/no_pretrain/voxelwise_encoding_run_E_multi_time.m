...
for sub = 1:length(num_subjects)
    ...
    dataroot = ['/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/GUMP_test/encoding_analysis/original_prednet/multi_time_result/2second_50frame/',layer_name, '/E/'];
    ...
    for run = 5:8 
        ...
        save_file_name = fullfile(dataroot, subject_id, [subject_id,'_lambda5_avg_E_run_', num2str(run), '.dtseries.nii']);
    ...
    save_file_name = fullfile(dataroot, subject_id, [subject_id,'_lambda5_avg_E.dtseries.nii']);
