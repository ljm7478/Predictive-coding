...
% 
% % Define paths
dataroot = '/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/01_gump/result/prednet_CNN_like_ver2_A_pass/h5/layer4/1frame_prediction';
saveroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/GUMP_test/encoding_analysis/modified_prednet/result/layer4/C/';
...
% 
% % Frame counts for each seg (training: run1-4, testing: run5-8)
...
for lay = 1:length(layername)
    ...
    for seg = 1:num_train
        secpath = fullfile(dataroot, ['seg', num2str(seg), '_C.h5']);
...
srate = 25;  % CNN feature maps extracted at 25 frames/sec
p = [5, 16, 1, 1, 6, 0, 32];
...
for lay = 1:length(layername)
    ...
    for seg = 0:3  % Train runs (run1 - run4)
        disp(['Processing seg: ', num2str(seg)]);
        secpath = fullfile(dataroot, ['seg', num2str(seg), '_C.h5']);
...
% CNN layer labels
layername = {'/C0'; '/C1'; '/C2'; '/C3'}; 
dataroot = saveroot; %'/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/GUMP_test/encoding_analysis/modified_prednet/result/layer4/C/';
