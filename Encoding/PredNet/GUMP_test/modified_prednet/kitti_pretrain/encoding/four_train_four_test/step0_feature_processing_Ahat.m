%% Voxel-wise Encoding Pipeline for GUMP Dataset
%
...
%
% Adapted from Budapest pipeline for GUMP dataset
...
FEATURE_TYPE = 'Ahat';  % 'Ahat' for predictions, 'E' for errors

% --- Paths (GUMP) ---
FEATURE_DATA_ROOT = '/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/01_gump/result/CNN_like/h5/pt_kitti_ft_Lall_nt150/layer4';
...
SAVE_ROOT = fullfile('/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/GUMP_test/encoding_analysis/modified_prednet/kitti_pretrain_result/four_train_four_test_PCAcap/layer4/', FEATURE_TYPE);
...
% --- Data Split (GUMP: 8 runs) ---
% seg0-seg3: training, seg4-seg7: testing
train_runs = 0:3;   % seg0, seg1, seg2, seg3
test_runs = 4:7;    % seg4, seg5, seg6, seg7

% --- PCA Parameters ---
% CRITICAL FOR PREVENTING OVERFITTING:
%
% GUMP fMRI: TR=2s, ~450 TRs per run x 4 runs = ~1800 TRs training
% If PCA components > training TRs -> underdetermined regression -> overfitting
%
% Rule of thumb: MAX_PCA_COMPONENTS <= fMRI_TRs / 3

PCA_VARIANCE_THRESHOLD = 0.90;  % Target variance to explain
MAX_PCA_COMPONENTS = 500;       % Hard cap (must be < training TRs)

% --- GUMP-specific Parameters ---
STIMULUS_FRAMERATE = 25;  % Hz (GUMP frame rate)
FMRI_TR = 2.0;            % seconds (GUMP TR)
...
fprintf('         PCA Cap: %d components (prevents overfitting)\n', MAX_PCA_COMPONENTS);
fprintf('%s\n', repmat('=', 1, 70));
...
for lay = 1:num_layers
    ...
    fprintf('Computing PCA (with cap=%d)...\n', MAX_PCA_COMPONENTS);
...
% 
% fprintf('\n%s\n', repmat('=', 1, 70));
% fprintf('[Step 3] Running Voxel-wise Encoding for Each Subject\n');
% fprintf('%s\n', repmat('=', 1, 70));
% 
...
%     fprintf('\n--- Starting Encoding for Subject: %s (%d/%d) ---\n', subject, subj_idx, length(subjects));
% 
...
%     if ~exist(fmri_per_run_path, 'file')
%         warning('Missing per-run fMRI data for %s. Skipping.', subject);
%         continue;
%     end
...
%         end
%     end
%     fmri_train_data = cat(2, fmri_train_cell{:}); % (Voxels x TrainingTime)
% 
%     fprintf('  Training fMRI: [%d voxels x %d TRs]\n', size(fmri_train_data, 1), size(fmri_train_data, 2));
% 
...
% 
%         fprintf('    Y_train: [%d TRs x %d features]\n', size(Y_train, 1), size(Y_train, 2));
% 
