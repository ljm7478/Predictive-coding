...
clc;clear
% dir
dataroot = '/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/03_Titanic/result/GUMP/4layers';
...
for i = 0:3
    disp(['start avg for layer', num2str(i)])
    ...
    for feature_idx = 1:numel(features)
        ...
        if feature_idx == 1
            ...
            %load lay_feat
            disp(sprintf('load %s%d lay_feat for avg', feature, i))
...
for i = 0:3
    ...
    for feature_idx = 1:numel(features)
        ...
        if feature_idx == 1
            ...
            %load lay_feat
            disp(sprintf('load %s%d lay_feat for std', feature, i))
            
            secpath = [dataroot, '/', sprintf('Ahat%d.h5',i)];
            lay_feat = h5read(secpath,'/predimg');
            
            lay_feat_mean = h5read([folder_name, sprintf('Ahat%d_feature_maps_avg.h5', i)],'/data');
        ...
        elseif feature_idx == 2
            ...
            %load lay_feat_mean
            lay_feat_mean = h5read([folder_name, sprintf('E%d_feature_maps_avg.h5',i)],'/data');
        ...
        clear lay_feat_mean lay_feat_std
...
%make folder
folder_name = [saveroot,'/', 'X','/','4layers', '/']
if not(exist(folder_name,'dir'))
    mkdir(folder_name)
end
...
for i = 0:3
    ...
    for feature_idx = 1:numel(features)
        ...
        if feature_idx == 1
            feature = features{feature_idx}
            disp(sprintf('load %s%d svd', feature, i))
            
            disp(sprintf('load %s  mean and std', feature))
            lay_feat_mean = h5read([saveroot,'/', 'feature_maps','/', '4layers', '/',  sprintf('Ahat%d_feature_maps_avg.h5',  i)], '/data');
            disp(size(lay_feat_mean))
            
            lay_feat_std = h5read([saveroot,'/', 'feature_maps','/', '4layers', '/', sprintf('Ahat%d_feature_maps_std.h5', i)], '/data');
            ...
            for iter =  1  : Niter
                ...
                disp(['Iteration ', num2str(iter), ' completed for ', feature]);
            end
            clear lay_feat_mean lay_feat_std lay_feat
        ...
        elseif feature_idx == 2
            ...
            disp(sprintf('load %s mean and std',feature))
            lay_feat_mean = h5read([saveroot,'/', 'feature_maps','/', '4layers', '/',  sprintf('E%d_feature_maps_avg.h5',  i)], '/data');
            disp(size(lay_feat_mean))

            lay_feat_std = h5read([saveroot,'/', 'feature_maps','/', '4layers', '/', sprintf('E%d_feature_maps_std.h5',  i)], '/data');
            disp(size(lay_feat_std))
...
for i=0:3
    ...
    lay_feat_mean = h5read([saveroot, '/', 'feature_maps', '/',  '4layers', '/', sprintf('Ahat%d_feature_maps_avg.h5', i)], '/data');
    lay_feat_std = h5read([saveroot, '/','feature_maps', '/', '4layers', '/', sprintf('Ahat%d_feature_maps_std.h5', i)], '/data');
    load([saveroot,'/', 'X', '/','4layers', '/', sprintf('Ahat%d_feature_maps_svd_0.90.mat', i) ], 'B');
    ...
    save([saveroot,'/', 'X', '/', '4layers', '/', filename], 'ts', '-v7.3');
...
for i=0:3
    ...
    disp(sprintf('E%d_feature_maps_avg.h5', i))
    disp(sprintf('E%d_feature_maps_std.h5', i))
    
    %load svd 
    load([saveroot,'/', 'X', '/','4layers', '/',  sprintf('E%d_feature_maps_svd_0.90.mat', i) ], 'B');

    disp(sprintf('E%d_feature_maps_svd_0.90.mat', i))
...
for feature_idx = 1:numel(features)
    ...
    for i = 0:3
        %open ts
        load([saveroot,'/',  'X', '/', '4layers', '/', sprintf('%s%d_feature_maps_svd_0.90_hrf.mat',feature, i)], 'ts');
        ...
        for seg = 1:8
            ...
            segment_filename = [sprintf('%s%d_svd0.9hrf_seg%d.mat', feature, i, seg)];
            save([saveroot,'/',  'X', '/', '4layers', '/', segment_filename], 'segments', '-v7.3');
        end
...
for feature_idx = 1:numel(features)
    ...
    for layer = 0:3
        for seg = 1:8
            filename = sprintf('%s/%s%d_svd0.9hrf_seg%s.mat', svd_folder, feature, layer, num2str(seg))
   ...
   for feature_idx = 1:numel(features)
        ...
        for seg = 1:8
           ...
           for layer = 0:3
                ...
                folder_name = [saveroot,'/', 'W','/','4layers','/',sprintf('seg%d',seg), '/']
                if not(exist(folder_name,'dir'))
                    mkdir(folder_name)
                end
...
for i = 1:12
    ...
    for feature_idx = 1:numel(features)
        ...
        disp(feature); 
        for seg=1:8
            folder_name = [saveroot,'/', 'Y_hat','/','4layers', '/', sprintf('seg%d',seg),'/']
            if not(exist(folder_name,'dir'))
                mkdir(folder_name)
            end  
             for layer=0:3
                ...
                W = load([saveroot,'/', 'W', '/', '4layers', '/',  sprintf('seg%d',seg), '/', subject, '_', feature, num2str(layer), '_W_svd0.9_s3.mat']);
                name = 'Y_prediction';
                eval([name, '= X* W.W;']);
...
folder_name = [saveroot,'/', 'correlations','/', 'PCA', '/','10k', '/', 'total', '/']
if not(exist(folder_name,'dir'))
    mkdir(folder_name)
end
...
for i = 1:15
    ...
    for feature_idx = 1:numel(features)
        ...
        for layer= 0:3
            for seg = 1:8
                folder_name = [saveroot,'/', 'correlations','/', 'PCA', '/','10k', '/', sprintf('seg%d',seg),'/']
                if not(exist(folder_name,'dir'))
                    mkdir(folder_name)
                end 

                Y_pred = load([saveroot,'/', 'Y_hat','/','PCA', '/', '10k', '/', sprintf('seg%d',seg), '/', feature, '/',  strcat(subject,'_', feature , num2str(layer), '_pred_svd0.90.mat')]);
                Y_pred = Y_pred.Yhat;
    ...
    for seg = 1:8
        ...
        for feature_idx = 1:numel(features)
            ...
            for layer= 0:3
                ...
                % per seg 
                load([saveroot,'/', 'correlations','/', 'PCA', '/', '10k', '/', sprintf('seg%d',seg), '/', strcat(num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90_s3.mat')]);
                disp([sprintf('seg%d',seg), '/', strcat(num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90_s3.mat')]);
                
                % %total seg 
                % load([saveroot,'/', 'correlations','/', 'PCA', '/', '10k', '/', 'total', '/', strcat(num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90_s3.mat')]);
                % disp([sprintf('seg%d',seg), '/', strcat(num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90_s3.mat')]);
            ...
            % % Save as dtseries for total seg 
            % save_file_name = ([num2str(subject),'_', feature, '.dtseries.nii']);
            % ciftisavereset(X, fullfile(saveroot, 'dt_result', '/', 'PCA','/', 'total', '/',save_file_name), 'wb_command');
...
%         end 
        ...
        save([saveroot, '/', 'correlations', '/', 'PCA', '/', '10k', '/', sprintf('seg%d',seg),  '/', sprintf('%s_avg_corr_svd0.90_s3.mat', feature)], 'Y_hat', '-v7.3');
        Y_hat = {};
    end
end 
%% make avg dtseries  - total sub + single seg
clc;clear
addpath(genpath('/local_raid1/01_software/HCPpipelines/global/matlab/cifti-matlab'));
...
%         end 
            for layer=0:3
                ...
                % Load correlation
                load([saveroot, '/', 'correlations', '/', 'PCA', '/', '10k', '/', sprintf('seg%d',seg),  '/', sprintf('%s_avg_corr_svd0.90_s3.mat', feature)]);
...
for feature_idx = 1:numel(features)
    ...
    save([saveroot, '/', 'correlations', '/', 'PCA', '/', '10k', '/','total',  '/', sprintf('%s_avg_corr_svd0.90_s3.mat', feature)], 'Y_hat', '-v7.3');
    Y_hat = {};
...
folder_name = [saveroot,'/', 'dt_result','/', 'PCA', '/', 'total', '/']
if not(exist(folder_name,'dir'))
    mkdir(folder_name)
end
...
for feature_idx = 1:numel(features)
    ...
    disp(feature)
            ...
            for layer=0:3
                ...
                % Load correlation
                load([saveroot, '/', 'correlations', '/', 'PCA', '/', '10k', '/', 'total', '/', sprintf('%s_avg_corr_svd0.90_s3.mat', feature)]);
...
%% below code is for making Y for 59k and 10k 
%% Train Encoding with GUMP fMRI processing w/ fMRIPrep + 3mm smoothing - 59k
% Load Y
clc;clear 
addpath(genpath('/local_raid1/01_software/HCPpipelines/global/matlab/cifti-matlab'));
saveroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/500SUM_CIT_trained/GUMP_test';
...
%% Train Encoding with GUMP fMRI processing w/ fMRIPrep + 3mm smoothing - 10k
% Load Y
clc;clear 
