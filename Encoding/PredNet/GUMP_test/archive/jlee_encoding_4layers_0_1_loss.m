...
% dir
dataroot = '/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/03_Titanic/result/GUMP/4layers_32channel_0.0-1.0loss';
saveroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/GUMP_test';

%make folder
folder_name = [saveroot,'/', 'feature_maps','/', '4layers_32channel_0.0-1.0loss', '/']
...
%make folder
folder_name = [saveroot,'/', 'X', '/', '4layers_32channel_0.0-1.0loss', '/']
if not(exist(folder_name,'dir'))
    mkdir(folder_name)
end
...
for lay = 3 
    ...
    lay_feat_mean = h5read(fullfile(saveroot,'/', 'feature_maps','/', '4layers_32channel_0.0-1.0loss', '/',sprintf('Ahat%d_feature_maps_avg.h5',  lay)), '/data');
    lay_feat_std = h5read(fullfile(saveroot,'/', 'feature_maps','/', '4layers_32channel_0.0-1.0loss', '/', sprintf('Ahat%d_feature_maps_std.h5',  lay)), '/data');
...
for lay = 0:3 
    ...
    lay_feat_mean = h5read(fullfile(saveroot,'/', 'feature_maps','/', '4layers_32channel_0.0-1.0loss', '/',sprintf('E%d_feature_maps_avg.h5',  lay)), '/data');
    lay_feat_std = h5read(fullfile(saveroot,'/', 'feature_maps','/', '4layers_32channel_0.0-1.0loss', '/', sprintf('E%d_feature_maps_std.h5',  lay)), '/data');
...
svd_folder = [saveroot,'/', 'X','/', '4layers_32channel_0.0-1.0loss', '/'];
...
for lay=1:3
    ...
    lay_feat_mean = h5read([saveroot, '/', 'feature_maps','/', '4layers_32channel_0.0-1.0loss', '/',sprintf('Ahat%d_feature_maps_avg.h5', lay)], '/data');
    disp(sprintf('Ahat%d_feature_maps_avg.h5', lay))

    lay_feat_std = h5read([saveroot, '/', 'feature_maps','/', '4layers_32channel_0.0-1.0loss', '/', sprintf('Ahat%d_feature_maps_std.h5', lay)], '/data');
...
for lay=1:3
    ...
    lay_feat_mean = h5read([saveroot, '/', 'feature_maps', '/','4layers_32channel_0.0-1.0loss', '/',  sprintf('E%d_feature_maps_avg.h5',lay)], '/data');
    disp(sprintf('E%d_feature_maps_avg.h5', lay))

    lay_feat_std = h5read([saveroot, '/','feature_maps', '/','4layers_32channel_0.0-1.0loss', '/', sprintf('E%d_feature_maps_std.h5', lay)], '/data');
   ...
   for feature_idx = 1:numel(features)
        ...
        for seg = 1:8
           ...
           for layer = 0:3
                ...
                folder_name = [saveroot,'/', 'W','/','4layers_32channel_0.0-1.0loss','/','10k', '/',sprintf('seg%d',seg), '/']
                if not(exist(folder_name,'dir'))
                    mkdir(folder_name)
                end
   ...
   for feature_idx = 1:numel(features)
        ...
        for seg = 1:8
           ...
           for layer = 0:3
                ...
                W = load([saveroot,'/', 'W','/', '4layers_32channel_0.0-1.0loss', '/', '10k', '/', sprintf('seg%d',seg), '/', num2str(subject), '_', feature, num2str(layer), '_W_svd0.9_s3.mat']);
                name = ['Y_pred_', feature, num2str(layer)] 
                eval([name, '= X*W.W;']);
                
                folder_name = [saveroot,'/', 'Y_hat','/','4layers_32channel_0.0-1.0loss', '/', '10k', '/',sprintf('seg%d',seg), '/']
                if not(exist(folder_name,'dir'))
                    mkdir(folder_name) 
                end
...
for i = 1:15
    ...
    for feature_idx = 1:numel(features)
        ...
        for layer= 0:3
            for seg = 1:8
                folder_name = [saveroot,'/', 'correlations','/',  '4layers_32channel_0.0-1.0loss', '/', '10k', '/', sprintf('seg%d',seg)]
                if not(exist(folder_name,'dir'))
                    mkdir(folder_name)
                end

                Y_pred = load([saveroot,'/', 'Y_hat','/','4layers_32channel_0.0-1.0loss','/','10k', '/', sprintf('seg%d',seg), '/', strcat(num2str(subject),'_', feature , num2str(layer),'_pred_svd0.9_s3', '.mat')]);
                Y_pred = Y_pred.Yhat;
                ...
                save([saveroot,'/', 'correlations','/', '4layers_32channel_0.0-1.0loss','/','10k', '/',sprintf('seg%d',seg), '/', name], 'correlations', '-v7.3');
...
folder_name = [saveroot,'/', 'correlations','/', '4layers_32channel_0.0-1.0loss','/', 'total', '/']
if not(exist(folder_name,'dir'))
    mkdir(folder_name)
end
...
for i = 1:15
    ...
    for feature_idx = 1:numel(features)
        ...
        for layer= 0:3
            for seg = 1:8
                Y_hat = load([saveroot,'/', 'Y_hat','/','4layers_32channel_0.0-1.0loss','/', '10k', '/',sprintf('seg%d',seg), '/', strcat(num2str(subject),'_', feature , num2str(layer),'_pred_svd0.9_s3', '.mat')]);
...
%             cii = rmfield(cii, 'cdata');
            ...
            %load correaltion
            load([saveroot,'/', 'correlations','/','4layers_32channel_0.0-1.0loss','/', 'total', '/', num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90_s3']);
            disp([saveroot,'/', 'correlations','/','4layers_32channel_0.0-1.0loss','/', 'total', '/', num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90_s3']);
...
for feature_idx = 1:numel(features)
    ...
    disp(feature)
    for seg = 1:8
%         folder_name = [saveroot,'/', 'dt_result','/', '4layers_32channel_0.0-1.0loss','/', sprintf('seg%d',seg),'/']
%         if not(exist(folder_name,'dir'))
%             mkdir(folder_name)
%         end 
        for layer= 0:3
            ...
            for i = 1:15
                ...
                % Load correlation
                load([saveroot,'/', 'correlations','/', '4layers_32channel_0.0-1.0loss','/','10k','/', 'total', '/', strcat(num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90_s3.mat')]);

                sum_values = sum_values + correlations';
            end
        Y_hat{layer+1} = sum_values / 15;
        end
        save([saveroot, '/', 'correlations', '/', '4layers_32channel_0.0-1.0loss','/','10k', '/', 'total', '/', sprintf('%s_avg_corr_svd0.90_s3.mat', feature)], 'Y_hat', '-v7.3');
...
for feature_idx = 1:numel(features)
    ...
    disp(feature)
    for seg = 1:8
        folder_name = [saveroot,'/', 'dt_result','/', '4layers_32channel_0.0-1.0loss','/','10k', '/', 'total', '/']
        if not(exist(folder_name,'dir'))
            mkdir(folder_name)
        end 
            for layer=0:3
                ...
                % Load correlation
                load([saveroot, '/', 'correlations', '/', '4layers_32channel_0.0-1.0loss','/','10k', '/', 'total', '/', sprintf('%s_avg_corr_svd0.90_s3.mat', feature)]);
            ...
            ciftisavereset(X, fullfile(saveroot,'/', 'dt_result','/', '4layers_32channel_0.0-1.0loss','/','10k', '/', 'total', '/',  save_file_name), 'wb_command');
