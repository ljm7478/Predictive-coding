%% Step2: Concate X
saveroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/GUMP_test/prediction_time_delay';
dataroot = '/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/03_Titanic/result/GUMP/4layers_32channel/h5';
...
for i = 1:3
    ...
    for feature_idx = 1:numel(features)
        ...
        for seg = 1:8
            ...
            secpath = sprintf([dataroot,'/run%d/%s%d.h5'], seg, feature, i);
            ...
            for j = length(time_delay)
               ...
               td = time_delay{j};
               concat_lay_feat = horzcat(lay_feat(1:end-25*td,:), lay_feat(25*td+1:end,:)); 
               
               h5_name = sprintf([saveroot, '/X/%s_time_delayed_feature_maps.h5'], feature);
...
for i = 0:3
    ...
    %open ts
    load([saveroot,'/', 'X/4layers_32channel', '/', 'PCA', '/',sprintf('%s%d_feature_maps_svd_0.90_hrf.mat', feature, i)], 'ts');
