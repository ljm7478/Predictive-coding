...
for i = 0:3
    X = cell(1, 7);
    for seg = 1:7
        filename = sprintf('%s/X/No_PCA/Ahat/Ahat%d_hrf_seg%d.mat', saveroot, i, seg);
...
for i = 1:15
    ...
    for feature=0:3
        ...
        name = [subject, '_', 'Ahat', num2str(feature), '_W_s3.mat'];
        save([saveroot,'/', 'W','/','No_PCA','/', '10k', '/', 'Ahat','/', name], 'W', '-v7.3');  
    end
end
...
for i = 1:15
    ...
    for feature=0:3
        ...
        save([saveroot,'/', 'W','/','No_PCA','/', '10k', '/', 'E', '/', name], 'W', '-v7.3');  
    end
end
...
for i = 0:3
    ...
    filename = sprintf('%s/X/No_PCA/E/E%d_hrf_seg%d.mat', saveroot, i, seg);
    loaded_data = load(filename);
    name = sprintf('E%d_8', i);
    E_concatenated.(name)= loaded_data.segments;
    eval([name, ' = E_concatenated.(name);']);
end
...
for i = 4
    ...
    for feature=0:3
        E_field_name = sprintf('E%d_8', feature);
        E_argument = E_concatenated.(E_field_name); 
        
        W = load([saveroot,'/', 'W','/','No_PCA','/', '10k', '/', subject, '_', 'E', num2str(feature), '_W_s3.mat']);
        name = ['Y_pred8_sub',num2str(i),'_', 'E', num2str(feature)]; 
        eval([name, '= E_argument* W.W;']);

        %save Y_hat
        Yhat = eval(name);
        name2 = strcat(subject, '_', 'E', num2str(feature), '_s3.mat');
        save([saveroot,'/', 'Y_hat','/','No_PCA', '/', 'E', '/',  name2], 'Yhat', '-v7.3');
    end
end
...
for i = 4
    ...
    for feature= 0:3
        ...
        Y_pred = load([saveroot,'/', 'Y_hat','/','No_PCA', '/','E', '/', subject,'_', 'E' , num2str(feature), '_s3', '.mat']);
        Y_pred = Y_pred.Yhat;
        ...
        name = strcat(subject, '_', 'E', num2str(feature), '_s3', '.mat');
        save([saveroot,'/', 'correlations','/', 'No_PCA', '/',  'E', '/',name], 'correlations', '-v7.3');
        ...
        name = strcat(subject, '_', 'E', num2str(feature), '_s3', '.mat');
        save([saveroot,'/', 'p_values','/', 'No_PCA', '/', 'E', '/', name], 'p_values', '-v7.3');
    end
end
...
%         cii = rmfield(cii, 'cdata');
        ...
        %load correaltion
        load([saveroot,'/', 'correlations','/','No_PCA', '/', '10k', '/','E','/', subject, '_', 'E', num2str(feature), '_s3.mat']);
        disp([saveroot,'/', 'correlations','/','No_PCA', '/', '10k', '/','E','/',subject, '_', 'E', num2str(feature), '_s3.mat']);
    ...
    ciftisavereset(X, fullfile(saveroot, 'dt_result', 'No_PCA','10k', '/', 'E','/',save_file_name), 'wb_command');
end
%% Step7: make dtseries - Ahat
clc;clear
addpath(genpath('/local_raid1/01_software/HCPpipelines/global/matlab/cifti-matlab'));
addpath(genpath('/local_raid1/01_software/toolboxes/cifti-matlab'));
%dir
