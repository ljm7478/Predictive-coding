...
% secpath = [dataroot, '/', 'total', '/', 'pre1', '.h5'];
...
h5create([saveroot,'/','total_500SUM_PredNet_feature_maps_hrf.h5'],'/data',...
    [size(ts)],'Datatype','single');
h5write([saveroot,'/','total_500SUM_PredNet_feature_maps_hrf.h5'], '/data', ts);
toc   % (123.66 sec)
%% Step4: divided into seg 1-6 (approx. 15min each)
clc; clear
% dir
dataroot = '/local_raid2/03_user/jungmin/01_Encoding/01_HWen_Encoding/stimuli/PredNet/02_FG_pretrained_test_500SUM_PredNet/raw_feature_maps';
saveroot = '/local_raid2/03_user/jungmin/01_Encoding/01_HWen_Encoding/stimuli/PredNet/02_FG_pretrained_test_500SUM_PredNet/feature_processed_encoding/X';
...
%open ts
secpath = [saveroot,'/','total_500SUM_PredNet_feature_maps_hrf.h5'];
...
for i = 1:6
    ...
    filename = [saveroot, '/', 'total_segment', num2str(i), '.h5'];
...
%% Plot the image
% figure
% imagesc(AlexNet_lay_feat1);
% figure
% imagesc(PredNet_lay_feat1);

%% Step5: voxelwise encoding
clc;clear
addpath(genpath('/local_raid1/01_software/toolboxes/cifti-matlab'))

%dir
dataroot = '/local_raid2/03_user/jungmin/01_Encoding/01_HWen_Encoding/stimuli/PredNet/02_FG_pretrained_test_500SUM_PredNet/feature_processed_encoding';
saveroot = '/local_raid2/03_user/jungmin/01_Encoding/01_HWen_Encoding/stimuli/PredNet/02_FG_pretrained_test_500SUM_PredNet/feature_processed_encoding';

%% Step6: Load fmri responses -NNDB - ICA ver.
...
for i = 1:6
    secpath = [saveroot, '/', 'total_segment', num2str(i), '.h5'];
    lay_feat = h5read(secpath,'/data');
...
% Y_fmri for subject 1-5
disp('Load Y_fmri for subject 1-5')

Y1_1_4 = sub1_cii.cdata(:,1:3647); 
Y2_1_4 = sub2_cii.cdata(:,1:3647); 
Y3_1_4 = sub3_cii.cdata(:,1:3647); 
Y4_1_4 = sub4_cii.cdata(:,1:3647); 
Y5_1_4 = sub5_cii.cdata(:,1:3647); 
...
for i = 2:5
    ...
    Y_name = ['Y', num2str(i), '_1_4'];
    Y = eval(Y_name);
    
    [W, Rmat, Lambda] = voxelwise_encoding(X1_4, Y', lambda, nfold);

    % save W (feature x voxel)
    name = strcat('sub', num2str(i), '_W.mat');
    save([saveroot,'/', 'W','/', name], 'W', '-v7.3');
    
    % Clear unnecessary variables to save memory
    clear Y_name Y;
    clear W Rmat Lambda;
    disp('Clear unnecessary variables') 
end

%% Step7: Load fmri responses -NNDB - fMRIPrep + 6mm smoothing (run1-3 concat then split into test)
clc; clear;
% Load Y
num_subjects = 5;
num_runs = 3;
...
for subject = 1:num_subjects
    ...
    for run = 1:num_runs
        ...
        if run == 3
            ...
            files = dir(fullfile(folder_path, '*.nii'));
...
for i = 1:5
    ...
    % save W (feature x voxel)
    name = strcat('sub', num2str(i), '_W_ciftify_6mm.mat');
    save([saveroot,'/', 'W','/', name], 'W', '-v7.3');
    
    % Clear unnecessary variables to save memory
    clear Y_name Y;
    clear W Rmat Lambda;
    disp('Cleared unnecessary variables') 
end

%% Step8: make predicted BOLD for test 
%dir
saveroot = '/local_raid2/03_user/jungmin/01_Encoding/01_HWen_Encoding/stimuli/PredNet/02_FG_pretrained_test_500SUM_PredNet/feature_processed_encoding';
...
% get X_predict using W from train set
% W = load([saveroot,'/', 'W','/', 'sub1', '_W.mat']);
% X_56_sub1 = Y_56*W.W;

for i = 1:5
    W = load([saveroot,'/', 'W','/', 'sub', num2str(i), '_W.mat']);
    name =['Y_pred56_sub',num2str(i)]; 
    eval([name, '= X_56*W.W;']);
end

% get test X (fMRI)
Y1_56 = sub1_cii.cdata(:,3648:end); 
Y2_56 = sub2_cii.cdata(:,3648:end); 
Y3_56 = sub3_cii.cdata(:,3648:end); 
Y4_56 = sub4_cii.cdata(:,3648:end); 
Y5_56 = sub5_cii.cdata(:,3648:end); 

% correaltion between X_predic and actual X for test
for i = 1:5
    Yactual_name = ['Y', num2str(i), '_56'];
    Y_actual = eval(Yactual_name);
    
    Ypred_name = ['Y_pred56_sub',num2str(i)]; 
    Y = eval(Ypred_name);
    
    [correlations, p_values] = auto_corr(Y_actual',Y);
    corr_name = ['corr_sub',num2str(i)]; 
    eval([corr_name, '= correlations;']);
end
...
%% Step9: make dtseries 
addpath(genpath('/local_raid1/01_software/toolboxes/cifti-matlab'))
saveroot = '/local_raid2/03_user/jungmin/01_Encoding/01_HWen_Encoding/stimuli/PredNet/02_FG_pretrained_test_500SUM_PredNet/feature_processed_encoding/dt_result/';
...
%save as dtseries
save_file_name='sub1_one_total_500SUM_PredNet_test56.dtseries.nii';
...
%save as dtseries
save_file_name='sub2_total_500SUM_PredNet_test56.dtseries.nii';
ciftisavereset(sub2_cii, [saveroot, save_file_name], 'wb_command');
...
%save as dtseries
save_file_name='sub3_total_500SUM_PredNet_test56.dtseries.nii';
ciftisavereset(sub3_cii, [saveroot, save_file_name], 'wb_command')
...
%save as dtseries
save_file_name='sub4_total_500SUM_PredNet_test56.dtseries.nii';
ciftisavereset(sub4_cii, [saveroot, save_file_name], 'wb_command')
...
%save as dtseries
save_file_name='sub5_total_500SUM_PredNet_test56.dtseries.nii';
ciftisavereset(sub5_cii, [saveroot, save_file_name], 'wb_command')
