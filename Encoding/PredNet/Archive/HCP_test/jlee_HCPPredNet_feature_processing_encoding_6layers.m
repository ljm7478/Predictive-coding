...
clc;clear
% dir
dataroot = '/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/03_Titanic/result/HCP/6layers_32channel';
...
for i = 0:5
    ...
    for feature_idx = 1:numel(features)
        ...
        secpath = [dataroot, '/', 'h5', '/', sprintf('%s%d.h5', feature, i)];
...
%dir
dataroot = '/combinelab/03_user/jungmin/01_project/02_PredNet/jmPNET/data/03_Titanic/result/HCP/6layers';
saveroot = '/combinelab/03_user/jungmin/01_project/01_Encoding/01_HWen_Encoding/PredNet/Titanic_trained/HCP_test';
...
for i = 0:5
    ...
    for feature_idx = 1:numel(features)
        ...
        lay_feat_mean = h5read(fullfile(saveroot, 'feature_maps', '/',sprintf('%s%d_feature_maps_avg.h5', feature, i)), '/data');
        lay_feat_std = h5read(fullfile(saveroot, 'feature_maps', '/', sprintf('%s%d_feature_maps_std.h5', feature, i)), '/data');
        
        % Load lay_feat
        disp('load lay_feat')
        secpath = fullfile(dataroot, '/', 'h5', '/', sprintf('%s%d.h5', feature, i));
        lay_feat = h5read(secpath, '/predimg');
...
for i = 0:5
    ...
    %save
    h5create([saveroot, '/', 'feature_maps', '/', sprintf('%s%d_1-4_concatenated_feature.h5', feature_name, i)],'/data',...
        [size(X_concat)],'Datatype','single');
    h5write([saveroot,'/','feature_maps', '/',  sprintf('%s%d_1-4_concatenated_feature.h5', feature_name, i)],'/data',X_concat);
end
...
for i = 0:5
    ...
    for feature_idx = 1:numel(features)
        ...
        secpath = [saveroot,'/','feature_maps', '/6layers/',  sprintf('%s%d_1-4_concatenated_feature.h5', feature, i)];
        lay_feat = h5read(secpath,'/data');
        
        %load svd
        load([saveroot,'/', 'X', '/','PCA', '/6layers/', sprintf('%s%d_feature_maps_svd_0.90.mat', feature, i)], 'B');
        ...
        save([saveroot,'/', 'X', '/', 'PCA', '/',  filename], 'ts', '-v7.3');
        
        disp([saveroot,'/', 'X', '/', 'PCA', '/',  filename])
...
for i = 0:5
    ...
    %open ts
    load([saveroot,'/', 'X', '/', 'PCA', '/',sprintf('%s%d_feature_maps_svd_0.90_hrf.mat', feature, i)], 'ts');
    ...
    end_indices = cumsum(Nv);
        ...
        for seg = 1:4
            ...
            save([saveroot,'/', 'X', '/', 'PCA', '/', segment_filename], 'segments', '-v7.3');
        end
    end
end
...
for feature_idx = 1:numel(features)
    ...
    for layer = 0:5
        for seg = 1:4
            filename = sprintf('%s/X/PCA/6layers_32channel/%s%d_feature_maps_svd_0.90_hrf_seg%d.mat', saveroot, feature, layer, seg);
...
load([saveroot, '/' , 'Y', '/', 'fMRI_59k_cortex_hcp_clean_ver.mat']);
subjectIDs = [100610, 116726, 135124, 156334, 169343, 178243, 191841, 201515, 249947, 380036, 463040, 725751, 818859, 901139, 102311, 118225, 137128, 157336, 169444, 178647, 192439, 203418, 251833, 381038, 467351, 601127, 732243, 825048, 901442, 995174, 102816, 125525, 140117, 158035, 169747, 180533, 192641, 204521, 257845, 385046, 617748, 826353, 905147, 104416, 126426, 144226, 158136, 171633, 181232, 193845, 205220, 263436, 389357, 525541, 627549, 751550, 833249, 910241, 105923, 145834, 159239, 172130, 195041, 209228, 283543, 393247, 638049, 757764, 859671, 926862, 108323, 128935, 146129, 162935, 173334, 182436, 196144, 212419, 318637, 395756, 541943, 644246, 765864, 861456, 927359, 109123, 130114, 146432, 164131, 175237, 182739, 197348, 214019, 320826, 397760, 547046, 654552, 770352, 871762, 942658, 111312, 130518, 146735, 164636, 176542, 185442, 198653, 214524, 330324, 401422, 671855, 771354, 872764, 943862, 111514, 131722, 146937, 165436, 177140, 186949, 199655, 221319, 346137, 406836, 562345, 680957, 782561, 878776, 951457, 114823, 132118, 148133, 167036, 177645, 187345, 200210, 233326, 352738, 412528, 572045, 690152, 783462, 878877, 958976, 115017, 134627, 150423, 167440, 177746, 191033, 200311, 239136, 360030, 429040, 573249, 706040, 789373, 898176, 966975, 115825, 134829, 155938, 169040, 178142, 191336, 200614, 246133, 365343, 436845, 581450, 724446, 814649, 899885, 971160];
features = {'Ahat' , 'E'};
    ...
    for feature_idx = 1:numel(features)
        ...
        for seg = 1:4
           ...
           for layer = 0:5
                ...
                folder_name = [saveroot,'/', 'W','/','PCA','/', '6layers', '/', '59k', '/', sprintf('seg%d',seg)]
...
for i = 1:15
    ...
    for feature_idx = 1:numel(features)
        ...
        for layer=0:5
            ...
            %load correaltion
            load([saveroot,'/', 'correlations','/','PCA', '/', 'total', '/', num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90']);
            disp([saveroot,'/', 'correlations','/','PCA', '/', 'total', '/', num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90']);
...
%         end 
            for layer=0:5
                ...
                % Load correlation
                load([saveroot, '/', 'correlations', '/', 'PCA', '/', sprintf('seg%d', seg), '/', sprintf('%s_avg_corr_svd0.90.mat', feature)]);
