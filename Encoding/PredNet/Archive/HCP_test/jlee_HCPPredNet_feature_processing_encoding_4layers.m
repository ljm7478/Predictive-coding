...
%make folder
folder_name = [saveroot,'/', 'feature_maps','/','4layers', '/']
...
for i = 0:3
    ...
    for feature_idx = 1:numel(features)
        ...
        lay_feat_mean = sum(lay_feat, length(dim)) / dim(4);
...
%% Step1.5: MOVIE1-4 concat before PCA - Prediction 
clc;clear
...
X = cell(4,3); % Initialize a cell array to store lay_feat matrices for each layer
...
for i = 0:3
    ...
    for feature_idx = 1:numel(features)
        ...
        lay_feat_mean = h5read(fullfile(saveroot,'/', 'feature_maps','/','4layers', '/',sprintf('%s%d_feature_maps_avg.h5', feature, i)), '/data');
        lay_feat_std = h5read(fullfile(saveroot,'/', 'feature_maps','/','4layers', '/', sprintf('%s%d_feature_maps_std.h5', feature, i)), '/data');
        
        % Load lay_feat
        disp('load lay_feat')
        secpath = fullfile(dataroot, sprintf('%s%d.h5', feature, i));
        lay_feat = h5read(secpath, '/predimg');
        ...
        X{i+1,feature_idx} = lay_feat; 
        disp(size(X{i+1,feature_idx}))
    end
...
for i = 0:3
    ...
    X_concat = cat(2, X{i+1, 1}, X{i+1, 2}, X{i+1, 3}, X{i+1, 4});
    eval([name, '= X_concat;']);
    
    %save
    h5create([saveroot, '/', 'feature_maps', '/','4layers', '/', sprintf('%s%d_1-4_concatenated_feature.h5', feature_name, i)],'/data',...
        [size(X_concat)],'Datatype','single');
    h5write([saveroot,'/','feature_maps', '/', '4layers', '/', sprintf('%s%d_1-4_concatenated_feature.h5', feature_name, i)],'/data', X_concat);
end

%% Step1.5: MOVIE1-3 concat before PCA - Error channel divide
clc;clear
...
for i = 0:3
    ...
    for feature_idx = 1:numel(features)
        ...
        % Step 1: Calculate mean and standard deviation
        disp('load mean and std')
        lay_feat_mean = h5read([saveroot,'/', 'feature_maps','/','4layers', '/', sprintf('%s%d_feature_maps_avg.h5', feature, i)], '/data');
        lay_feat_std = h5read([saveroot,'/', 'feature_maps','/','4layers', '/', sprintf('%s%d_feature_maps_avg.h5', feature, i)], '/data');
...
%make folder
folder_name = [saveroot,'/', 'X', '/', 'PCA', '/','4layers', '/']
if not(exist(folder_name,'dir'))
    mkdir(folder_name)
end
...
for i = 0:3
    ...
    for feature_idx = 1:numel(features)
        ...
        secpath = [saveroot,'/', 'feature_maps','/','4layers', '/', sprintf('%s%d_1-4_concatenated_feature.h5', feature, i)]
        ...
        save([folder_name, sprintf('%s%d_feature_maps_svd_0.90.mat', feature, i)], 'B', 's', '-v7.3');
...
for i = 0:3
    ...
    for feature_idx = 1:numel(features)
        ...
        %load svd
        load([saveroot,'/', 'X', '/', 'PCA', '/','4layers', '/', sprintf('%s%d_feature_maps_svd_0.90.mat', feature, i)], 'B');
        ...
        save([saveroot,'/', 'X', '/', 'PCA', '/','4layers', '/' , filename], 'ts', '-v7.3');
        
        disp([saveroot,'/', 'X', '/', 'PCA', '/','4layers', '/' , filename])
...
for i = 0:3
    ...
    %open ts
    load([saveroot,'/', 'X', '/', 'PCA', '/', '4layers', '/', sprintf('%s%d_feature_maps_svd_0.90_hrf.mat', feature, i)], 'ts');
    ...
    end_indices = cumsum(Nv);
        ...
        for seg = 1:4
            ...
            save([saveroot,'/', 'X', '/', 'PCA', '/', '4layers', '/', segment_filename], 'segments', '-v7.3');
        end
    end
end
...
for feature_idx = 1:numel(features)
    ...
    for layer = 0:3
        for seg = 1:4
            filename = sprintf('%s/X/PCA/4layers/%s%d_feature_maps_svd_0.90_hrf_seg%d.mat', saveroot, feature, layer, seg);
    ...
    for feature_idx = 1:numel(features)
        ...
        for seg = 1:4
           ...
           for layer = 0:3
                ...
                folder_name = [saveroot,'/', 'W','/','PCA','/','4layers', '/', sprintf('seg%d',seg)]
                if not(exist(folder_name,'dir'))
                    mkdir(folder_name)
                end
    ...
    for feature_idx = 1:numel(features)
        ...
        for seg=1:4
            ...
            for layer=0:3
                ...
                folder_name = [saveroot,'/', 'Y_hat','/','PCA', '/' ,'4layers', '/', sprintf('seg%d',seg)]
                if not(exist(folder_name,'dir'))
                    mkdir(folder_name)
                end
                
                W = load([saveroot,'/', 'W', '/', 'PCA',  '/', '4layers', '/', sprintf('seg%d',seg), '/',num2str(subject), '_', feature, num2str(layer), '_W_svd0.9.mat']);
...
for i = 1:15
    ...
    for feature_idx = 1:numel(features)
        ...
        for layer=0:3
            for seg = 1:4
                Y_hat = load([saveroot,'/', 'Y_hat','/','PCA', '/','4layers', '/', sprintf('seg%d',seg), '/', strcat(num2str(subject),'_', feature , num2str(layer), '_pred_svd0.90.mat')]);
                disp([saveroot,'/', 'Y_hat','/','PCA', '/', '4layers', '/', sprintf('seg%d',seg), '/', strcat(num2str(subject),'_', feature , num2str(layer), '_pred_svd0.90.mat')])
                X{seg} = Y_hat.Yhat;
            end
            ...
            folder_name = [saveroot,'/', 'correlations','/','PCA', '/' ,'4layers', '/', 'total', '/']
            if not(exist(folder_name,'dir'))
                mkdir(folder_name)
            end
...
for i = 1:15
    ...
    for feature_idx = 1:numel(features)
        ...
        for layer= 0:3
            for seg = 1:4
                folder_name = [saveroot,'/', 'correlations','/', 'PCA', '/', '4layers', '/', sprintf('seg%d',seg)]
                if not(exist(folder_name,'dir'))
                    mkdir(folder_name)
                end
                
                Y_pred = load([saveroot,'/', 'Y_hat','/','PCA', '/',  '4layers', '/', sprintf('seg%d',seg), '/', strcat(num2str(subject),'_', feature , num2str(layer),'_pred_svd0.90', '.mat')]);
                Y_pred = Y_pred.Yhat;
                ...
                name = strcat(num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90', '.mat')
                save([saveroot,'/', 'correlations','/', 'PCA', '/', '4layers', '/', sprintf('seg%d',seg), '/', name], 'correlations', '-v7.3');
...
%% Step7: make dtseries -single subject
clc;clear
...
for i = 1:15
    subject = subjectIDs(i)
    for seg = 1:4
%         folder_name = [saveroot,'/', 'dt_result','/', '4layers', '/', sprintf('seg%d',seg)]
%         if not(exist(folder_name,'dir'))
%             mkdir(folder_name)
%         end
        for feature_idx = 1:numel(features)
            ...
            for layer= 0:3
                ...
                %load correaltion
                load([saveroot,'/', 'correlations','/', 'PCA', '/', '4layers', '/', 'total', '/', strcat(num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90.mat')]);
            ...
            ciftisavereset(X, fullfile(saveroot, 'dt_result', '/',  '4layers', '/', 'total', '/',save_file_name), 'wb_command');
...
%         end 
        for layer= 0:3
            ...
            for i = 1:15
                ...
                % Load correlation
                load([saveroot,'/', 'correlations','/', 'PCA', '/', '4layers', '/', sprintf('seg%d',seg), '/', strcat(num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90.mat')]);
                disp([sprintf('seg%d',seg), '/', strcat(num2str(subject),'_', feature , num2str(layer),'_corr_svd0.90.mat')]);
        ...
        save([saveroot, '/', 'correlations', '/', 'PCA', '/', '4layers', '/', sprintf('seg%d',seg),  '/', sprintf('%s_avg_corr_svd0.90_s3.mat', feature)], 'Y_hat', '-v7.3');
        Y_hat = {};
    end
end 
%% make avg dtseries  - single seg
clc;clear
addpath(genpath('/local_raid1/01_software/HCPpipelines/global/matlab/cifti-matlab'));
...
for feature_idx = 1:numel(features)
    ...
    disp(feature)
    for seg = 1:4
%         folder_name = [saveroot,'/', 'dt_result','/', '4layers', '/', sprintf('seg%d',seg),'/', feature]
%         if not(exist(folder_name,'dir'))
%             mkdir(folder_name)
%         end 
            for layer=0:3
                ...
                % Load correlation
                load([saveroot, '/', 'correlations', '/', 'PCA', '/', '4layers', '/', sprintf('seg%d',seg),  '/', sprintf('%s_avg_corr_svd0.90_s3.mat', feature)]);
            ...
            ciftisavereset(X, fullfile(saveroot, 'dt_result', '4layers', '/', sprintf('seg%d',seg), '/',  save_file_name), 'wb_command');
...
for feature_idx = 1:numel(features)
    ...
    save([saveroot, '/', 'correlations', '/', 'PCA', '/','4layers', '/', 'total',  '/', sprintf('%s_avg_corr_svd0.90.mat', feature)], 'Y_hat', '-v7.3');
    Y_hat = {};

end

%% make avg dtseries - total
clc;clear
addpath(genpath('/local_raid1/01_software/HCPpipelines/global/matlab/cifti-matlab'));
...
folder_name = [saveroot,'/', 'dt_result','/', '4layers', '/', 'total' ,'/']
if not(exist(folder_name,'dir'))
    mkdir(folder_name)
end 
...
for feature_idx = 1:numel(features)
    ...
    disp(feature)
            ...
            for layer=0:3
                ...
                % Load correlation
                load([saveroot, '/', 'correlations', '/', 'PCA', '/', '4layers', '/','total', '/', sprintf('%s_avg_corr_svd0.90.mat', feature)]);
