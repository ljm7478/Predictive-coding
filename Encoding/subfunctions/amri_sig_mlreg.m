...
function para = amri_sig_mlreg(X, Y, M, x_v, y_v, m_v, para, opts )
    ...
    for iter=1:MaxIter
        ...
        if (rem(iter,IterNum)==0) && (~isempty(savefilename))
            ...
            % save([savefilename,'_iter',num2str(ceil(iter)), '.mat'], 'para');  %original code
            save([savefilename,'/','iter',num2str(ceil(iter)), '.mat'], 'para', '-v7.3');  %jmlee code for running visual_reconstruction_jmlee.m
        end
